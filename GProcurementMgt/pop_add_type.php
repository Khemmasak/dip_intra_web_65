...
$s_egp_type = $db->query("SELECT * FROM egp_type WHERE egp_type_id = '{$_GET[type_id]}' {$wh} ");	 
$a_egp_type = $db->db_fetch_array($s_egp_type);
...
<input name="id" 		type="hidden" id="id" 		value="<?php echo $_GET['type_id']; ?>" />
...
<h4 class="modal-title  color-white"><i class="fas fa-plus-circle"></i> <?php echo $txt_egp_type_add;?></h4>
</div>
...
<label for="topic1"><?php echo $txt_egp_type_code;?><span class="text-danger">*</span> : </label>        
<input name="topic1" type="text" id="topic1"  class="form-control" required value="<?php echo $a_egp_type['egp_type_code']; ?>" />	
</div>
</div>
<div class="form-group row">
<div class="col-md-12 col-sm-12 col-xs-12"> 
<label for="topic2"><?php echo $txt_egp_type_name;?><span class="text-danger">*</span> : </label>        
<input name="topic2" type="text" id="topic2"  class="form-control" required value="<?php echo $a_egp_type['egp_type_name']; ?>" />	
</div>
</div>
