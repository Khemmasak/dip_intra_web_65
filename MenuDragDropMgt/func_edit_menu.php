<?php
include("../EWT_ADMIN/comtop_pop.php"); 

$date = new DateTime();

$a_data = array_merge($_POST, $_FILES);
	
if($a_data['proc']=='Edit_Menu'){
	
$s_data = array();

	if(!empty($a_data['menu_show']))
	{
		$menu_show = $a_data['menu_show'];
	}
	else
	{
		$menu_show = 'N';
	}
	
$s_data['m_show']	=	$menu_show;  
$s_data['m_name']	=	$a_data['menu_title'];

//$a_data['page_id_array'];
//for($i=0; $i<count($a_data['page_id_array']); $i++){	
//$s_data['faq_cate_order']  = $i+1;	
//update('faq_category',$s_data,array('faq_cate_id'=>$a_data['page_id_array'][$i]));	
//}
//insert('menu_list',$s_data);	
//$_max = countmax('menu_list','m_id');							   
//echo($_max);	
update('menu_list',$s_data,array('m_id'=>$a_data['m_id'])); 
echo($a_data['m_id']);	
//}
//insert('menu_properties',$s_data);
$db->write_log("update","menu","แก้ไขเมนู".$a_data['menu_title']); 	
unset($a_data);
unset($s_data);

exit;
	} 
?>