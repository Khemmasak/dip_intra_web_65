<?php
	...
	function list_room($c_id,$depth){
		...
		while($subcate = $db->db_fetch_array($result_subcate)){
			...
			while($rec_q = $db->db_fetch_array($sql_q)){
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $rec_q[t_count]);
	...
	*/
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', "สถิติการเข้าเว็บบอร์ดจำนวนผู้อ่าน");
	
	$objPHPExcel->getActiveSheet()->getStyle('A1:D1')->applyFromArray($border_setting);

	$objPHPExcel->getActiveSheet()->setCellValue('A2', "รายการ");
	$objPHPExcel->getActiveSheet()->setCellValue('D2', "จำนวนผู้อ่าน");
