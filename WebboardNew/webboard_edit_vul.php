...
if($_POST["v_word"]){
	...
	##ตรวจข้อมูลซ้ำ##
	$query_ck_vul = "SELECT vulgar_id FROM w_vulgar WHERE vulgar_text = '".$_POST['v_word']."' AND ip_add = '".$_POST['v_type']."'";	
	$result_ck_vul = $db->query($query_ck_vul);
	$num_ck_vul = $db->db_num_rows($result_ck_vul);
	
	if($num_ck_vul == 0){
	
		$query_edit_vul ="UPDATE w_vulgar SET 
							vulgar_text='".$_POST['v_word']."',
							ip_add='".$_POST['v_type']."'
							WHERE vulgar_id='".$_GET['v_id']."'";
				 
		$edit_vul = $db->query($query_edit_vul);
		
		$db->write_log("edit","webboard vulgar","แก้ไข webboard vulgar ".$_POST['v_word']);
	}
...
<h4>แก้ไขคำที่ไม่สุภาพ/โฆษณา</h4>
<p></p> 
...
<ol class="breadcrumb">
	...
	<li><a href="webboard_vul_index.php?c_id=<?=$room_cid;?>">คำที่ไม่สุภาพ/โฆษณา</a></li>
	<li><a href="webboard_edit_vul.php?c_id=<?=$room_cid;?>">แก้ไขคำที่ไม่สุภาพ/โฆษณา</a></li>
</ol>
...
<div class="card ">
	<div class="card-header ewt-bg-color m-b-sm" >
		<div class="card-title text-left color-white">
			<p><h4>แก้ไขคำที่ไม่สุภาพ/โฆษณา</h4></p>
		</div>
	</div>
	<div class="card-body">
	...
	<form name="form1" method="post" action="#">
		...
		<?php
			$query_de = $db->query("SELECT * FROM w_vulgar  WHERE vulgar_id = '".$_GET['v_id']."'");
				...
				<table border="0" width="80%">
					<tr>
						...
						<td width="80%" style="padding: 10px">
							...
							<input type="text" name="v_word" value = "<?php echo $DE['vulgar_text'];?>" required>
					...
					<tr>
						...
						?>
							<div><input type = "radio" name = "v_type" value = "1" <?php echo $r_1; ?> required>&nbsp;คำไม่สุภาพ</div>
							<div><input type = "radio" name = "v_type" value = "2" <?php echo $r_2; ?>>&nbsp;โฆษณา</div>
						</td>
					</tr> 
				</table>
		...
		<div class="float-center text-center" style="top:18px;margin-bottom:10px;"> 
			<button id="submit_button" type="submit" class="btn btn-default btn-sm" >
				<i class="far fa-arrow-alt-circle-right"></i>&nbsp;แก้ไขคำที่ไม่สุภาพ/โฆษณา
			</button>
		</div>
	</form>
