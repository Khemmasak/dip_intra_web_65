...
$s_pquestion = $db->query("SELECT * FROM p_question WHERE q_id = '{$qid}'");
$PR = $db->db_fetch_array($s_pquestion);
...
<div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"><?=$lang_add_question_answerrank;?> <?php echo $qname; ?>  : <?php if($PR[q_anstype]=="A"){ echo "Radio Box"; } ?><?php if($PR[q_anstype]=="B"){ echo "Check Box"; } ?><?php if($PR[q_anstype]=="C"){ echo "Select Box"; } ?><?php if($PR[q_anstype]=="D"){ echo "Text Box"; } ?></h4>
        </div>
...
<input name="qname" type="hidden" id="qname" value="<?=$qname; ?>"/>
<input name="qid" type="hidden" id="qid" value="<?=$qid; ?>"/>
<input name="sel" type="hidden" id="sel" value="<?=$PR['q_anstype']; ?>"/>
...
<form name="form1" enctype="multipart/form-data" method="post" action="function_eqa1.php" onSubmit="return GoNext();"><br>
