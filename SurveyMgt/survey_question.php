...
include("../libraries/fpdf1.83/fpdf.php");
...
$FieldAns = mysqli_num_fields($SQLAns);
...
$Arr_Ans = array();
while( $loop<$FieldAns){
	 $FieldName = mysqli_field_name($SQLAns,$loop);
...
while($R=$db->db_fetch_array($SQL)){  
		...
		//Single Question
			   ...
			   $PartName1='ส่วนที่';
				...
				if($R[c_name] !=""){ 
					$data_sub.=" : ".preg_replace('<P>','',preg_replace('</P>','',$R[c_name])); 
				}
				...
				$pdf->Cell('180',$height,cov($data_sub),'LTR',1,'L',0); 
				 ...
				 if($R[c_title] !=""){ 
					 ...
					 $pdf->SetXY(15,$pdf->SetY);
					 $pdf->Cell('180',$height,cov($DescName1." : ").preg_replace('<P>','',preg_replace('</P>','',cov($R[c_title]))),'LRB',1,'L',0); 
				 }
				$pdf->SetXY(15,$pdf->SetY);
				...
				while($X = $db->db_fetch_array($SSS)){ 
						 $pdf->SetXY(15,$pdf->SetY+$height);
						 $pdf->Cell(10,$height,cov($X[q_name]),'LRB',0,'C',0); 
						 $data_qdes=preg_replace('<P>','',preg_replace('</P>','',$X[q_des])); 
	 ...
	 //Multi Question
				...
				while($X = $db->db_fetch_array($SSS)){
						...
						//$pdf->Image('mainpic/document_check.gif',$pdf->x,$pdf->SetY,png); 
