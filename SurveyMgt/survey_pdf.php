...
//เริ่มฟิลด์ที่ 2 เพราะว่า ฟิลด์ที่ 0 เป็น รหัส ฟิลด์ที่ 1 เป็น วันเวลา(เข้าใจผิดแล้ว)
//เริ่มฟิลด์ที่ 3 เพราะว่า ฟิลด์ที่ 0 เป็น รหัส ฟิลด์ที่ 1 เป็นรหัสสมาชิก ฟิลด์ที่ 2 เป็น วันเวลา
 $AnsIndex=3;
...
while($R=$db->db_fetch_array($SQL)){  
			   ...
			   $PartName1='ส่วนที่';
				...
				$pdf->Cell('180',$height,cov($data_sub),'LTR',1,'L',0); 
				 ...
				 if($R[c_title] !=""){ 
					 $DescName1='คำชี้แจง ';
					 $pdf->SetXY(15,$pdf->y); 
					 $pdf->Cell('180',$height,cov($DescName1)." : ".preg_replace($search, $replace, cov($R[c_title])),'LRB',1,'L',0); 
				 }
				$pdf->SetXY(15,$pdf->y);
				...
				$Arr_a_name=array();
				 while($Q = $db->db_fetch_array($SQL2)){ 
						// if( $chklast == $Count_SQL2) {   $LastLine=1; }else{ $LastLine=0; }
						 ...
						 $pdf->Cell($field_width,$height,preg_replace($search, $replace, cov($Q[a_name])),'LRB',$LastLine,'C',0); 
						 $Arr_a_name[]=$Q[a_name];
						 //$chklast++;
				}
				...
				while($X = $db->db_fetch_array($SSS)){ 
						 ...
						 $pdf->Cell(10,$height,preg_replace($search, $replace, cov($X[q_name])),'LRB',0,'C',0);  
								 ...
								 $DATA_Answer=$Arr_Ans[$AnsIndex];  
								  if($R[option1]=="A"){ 
										  if($DATA_Answer == $Arr_a_name[$i]){ 
											   $pdf->Cell($field_width,$height,'OK','LRB',cov($LastLine),'C',0); 
										  }else{
											  $pdf->Cell($field_width,$height,'','LRB',cov($LastLine),'C',0); 
										  }
								  }else{
										 if($DATA_Answer == $Arr_a_name[$i]){ 
											   $pdf->Cell($field_width,$height,'OK','LRB',cov($LastLine),'C',0); 
	 ...
	 //Multi Question
				...
				$data_sub=preg_replace($search, $replace, $X[q_des]);
				$pdf->Cell('180',$height,cov($data_sub),'LTR',1,'L',0); */
				$PartName1='ส่วนที่';
				$pdf->SetXY(15,$pdf->y);
				$pdf->SetFont('angsab', '',14);
				...
				while($X = $db->db_fetch_array($SSS)){
						...
						$re  = explode("#@form#img@#",$Arr_Ans[$AnsIndex]);
						$Arr_Ans[$AnsIndex] = $re[0];
						if($X[q_anstype]=="D"){ /////////////////////////////////////////////////////////////////// D Text
								...
								if($RrRows = $db->db_num_rows($SSS1)){
										$Z = $db->db_fetch_array($SSS1);
										 if($Z[a_other]=="S"){
											  $pdf->Cell('180',$height,preg_replace($search, $replace, cov($data_ansD)),'LR',1,'L',0); 
										 }else{
								 ...
								 $pdf->Cell('180',$height,cov($data_ansA),'LR',1,'L',0);
								  if($re[1] != ""){
								 $pdf->SetXY(15,$pdf->y);
								 $imge = $pdf->Image("../ewt/".$_SESSION["EWT_SUSER"]."/".$re[1],25,$pdf->y,20,20,'','');
								  $pdf->Cell('180','30','','LR',1,'L',0);
								 }
                                 $AnsIndex++;
								...
								while($Z = $db->db_fetch_array($SSS1)){
										...
										$data_ansB='          '.$answer_ex[0];
										 if($DATA_Answer != "" && $answer_ex[0] !=''){
													$pdf->SetXY(15,$pdf->y);
													$pdf->Cell('180',$height,cov($data_ansB),'LR',1,'L',0);
													if($answer_ex[1] != ""){
													 $pdf->SetXY(15,$pdf->y);
													 $imge = $pdf->Image("../ewt/".$_SESSION["EWT_SUSER"]."/".$answer_ex[1],25,$pdf->y,20,20,'','');
													  $pdf->Cell('180','30','','LR',1,'L',0);
													 }
										 }
