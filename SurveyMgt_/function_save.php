...
while($rrr = mysql_fetch_array($delete1)){
...
$RR = mysql_fetch_array($ex1);

$delete2 = $db->query("select * from p_question where c_id = '$rrr[c_id]'");
while($rrrr = mysql_fetch_array($delete2)){
...
$S = mysql_fetch_array($ex2);

$delete3 = $db->query("select * from p_ans where q_id = '$rrrr[q_id]'");
while($xx = mysql_fetch_array($delete3)){
