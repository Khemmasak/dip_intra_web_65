...
window.location.href="edit_asurvey.php?s_id=<?=$s_id;?>";
</script>
<?php
}
...
<div class="table-responsive">	
		...
		<tr _class="ewttablehead" style="background-color:#f5f5f5;">
		<td width="81%">
		<!--<input name="Eutt<?=$i;?>2" type="button" value="<?=$lang_survey_add_topic2;?>" class="btn btn-info  btn-lm " onClick="window.open('add.php?s_id=<?=$s_id; ?>','addp','width=750,height=650,resizable=1,status=0,scrollbars=1,location=0,top=40,left=200');">--->
		<a onclick="boxPopup('<?=linkboxPopup();?>pop_add.php?s_id=<?=$s_id;?>');" data-toggle="tooltip" data-placement="right" title="<?=$lang_survey_add_topic2; ?>" >
		<button type="button" class="btn btn-info btn-sm"   _onClick="window.open('edit_s.php?s_id=<?php echo $s_id; ?>','survey','width=500,height=200,resizable=1,status=0,scrollbars=1,location=0,top=40,left=200');" >
		<span class="glyphicon glyphicon-plus-sign"></span>&nbsp;<?=$lang_survey_add_topic2; ?>
		</button> 
		</a>
		</td>
		<td width="9%" class="text-center"><strong><?=$lang_survey_add;?></strong></td>
		<td width="10%" class="text-center"><strong><?=$lang_survey_rank;?></strong></td>
		<td colspan="2" class="text-center"><strong><?=$lang_survey_edit;?></strong></td>
		<td width="10%" class="text-center"><strong><?=$lang_survey_delete;?></strong></td>
		</tr>
<?php
$i = 0;
...
?>
		...
		<li><?=$lang_add3_part; ?> <?=$R['c_d']; ?>  : <?= $R['c_name']; ?></li>                  
		</td>
		<td width="9%" align="center" height="2"> 
		<!--<input name="Butt<?php echo $i; ?>" type="button" value="<?=$lang_add3_additem; ?>" class="btn btn-info  btn-sm " onClick="window.open('add_question.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>&type=<?=$R['c_gp']; ?>&pos=<?=$row+1; ?>','question','width=750,height=650,resizable=1,status=0,scrollbars=1,location=0,top=20,left=200');"> -->                	
		<a onclick="boxPopup('<?=linkboxPopup();?>pop_add_question.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>&type=<?=$R['c_gp']; ?>&pos=<?=$row+1; ?>');" data-toggle="tooltip" data-placement="right" title="<?=$lang_add3_additem; ?>" >
		<button type="button" class="btn btn-info btn-sm"   _onClick="window.open('edit_s.php?s_id=<?php echo $s_id; ?>','survey','width=500,height=200,resizable=1,status=0,scrollbars=1,location=0,top=40,left=200');" >
		<span class="glyphicon glyphicon-plus-sign"></span>&nbsp;<?=$lang_add3_additem; ?>
		</button> 
		</a>
		</td>
	
		<td width="10%" align="center" height="2">&nbsp;</td>
		<td width="10%" align="center" height="2">
		<!--<input name="Butt<?=$i; ?>2" type="button" value="<?=$lang_add3_edititem; ?> <?=$R['c_d']; ?>" class="btn btn-warning  btn-sm" onClick="window.open('edit.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>','part','width=750,height=650,resizable=1,status=0,scrollbars=1,location=0,top=40,left=200');">-->		
		<a onclick="boxPopup('<?=linkboxPopup();?>pop_edit.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>');" data-toggle="tooltip" data-placement="right" title="<?=$lang_add3_edititem; ?>&nbsp;<?=$R['c_d']; ?>" >
		...
		<input name="Cutt<?=$i; ?>" type="button" value="<?=$lang_add3_edititemquestion; ?>" class="btn btn-warning  btn-sm" onClick="window.open('edit_ans1.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>&type=<?=$R['c_gp']; ?>','ans1','width=750,height=650,resizable=1,status=0,scrollbars=1,location=0,top=20,left=200');">
		<!--<a onclick="boxPopup('<?=linkboxPopup();?>pop_edit_ans1.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>&type=<?=$R['c_gp']; ?>');" data-toggle="tooltip" data-placement="right" title="<?=$lang_add3_edititemquestion; ?>" >
		<button type="button" class="btn btn-warning btn-sm"  >
		<span class="glyphicon glyphicon-pencil"></span>&nbsp;<?//=$lang_add3_edititemquestion; ?>
		</button> 
		</a>-->
		<?php }else{ echo "&nbsp;"; } ?>
		</td>
		...
		<input  name="dell<?=$i; ?>" type="checkbox" id="dell<?=$i; ?>" value="<?=$R['c_id']; ?>">
        </label>
		</td>
		</tr>
<?php
...
?>
		...
		<input class="form-control numberint" name="pos<?=$i; ?><?=$t;?>" type="text" id="pos<?=$i;?><?=$t;?>" value="<?=$RR['q_pos'];?>" size="1" >
		<input name="qid<?=$i; ?><?=$t;?>" type="hidden" id="qid<?=$i;?><?=$t;?>" value="<?=$RR['q_id'];?>">
		</div>
		</td>
		<td align="center">
		<!--<input name="But<?//=$i; ?>" type="button" value="<?//=$lang_survey_edit_question; ?> <?//=$RR['q_name']; ?>" class="btn btn-warning  btn-sm" onClick="window.open('edit_question.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>&type=<?=$R['c_gp']; ?>&qid=<?=$RR['q_id']; ?>','ques','width=750,height=650,resizable=1,status=0,scrollbars=1,location=0,top=20,left=200');">-->
		<a onclick="boxPopup('<?=linkboxPopup();?>pop_edit_question.php?post=<?=$R['c_d']; ?>&path=<?=$R['c_id']; ?>&type=<?=$R['c_gp']; ?>&qid=<?=$RR['q_id']; ?>');" data-toggle="tooltip" data-placement="right" title="<?=$lang_survey_edit_question; ?>&nbsp;<?=$RR['q_name']; ?>" >
		<button type="button" class="btn btn-warning btn-sm"  >
		...
		<td align="center">
		<?php if($R['c_gp']=="N"){ ?>
		<input name="Cut<?=$i; ?>" type="button" value="<?=$lang_survey_edit_answer;?> <?=$RR['q_name']; ?>" class="btn btn-warning  btn-sm" onClick="window.open('edit_ans.php?qname=<?=$RR['q_name']; ?>&qid=<?=$RR['q_id']; ?>','ans','width=750,height=650,resizable=1,status=0,scrollbars=1,location=0,top=20,left=200');">
		<?php }else{ echo "&nbsp;"; } ?>
		</td>
		<td align="center">
		<label class="checkbox-inline">
		<input name="del<?=$i; ?><?=$t; ?>" type="checkbox" id="del<?=$i; ?><?=$t; ?>2" value="<?=$RR['q_id']; ?>">
		</label>
		</td>
		</tr>
<?php
...
?>
			 <tr style="background-color:#f5f5f5;" >
                  <td colspan="6" >
                    <?=$lang_add3_howmanyitems1; ?>
  
                    <?=$i+1; ?>
                
                    <?=$lang_add3_howmanyitems2; ?>
 
                    <?=$row; ?>
               
                    <?=$lang_add3_howmanyitems3; ?>
       
					<input name="ppp<?=$i; ?>" type="hidden" id="ppp<?=$i; ?>" value="<?=$row; ?>"></td>
            </tr>
		<?php	 }else{ ?>
			<tr>
                  <td colspan="6">&nbsp;&nbsp;&nbsp;<img src="dot.gif" width="7" height="7">&nbsp;<font color="#FF0000">
                    <?=$lang_add3_noitem;?>
                    </font>
...
<div class="panel-footer text-center" >
		...
		<!--<input name="SubmitT" type="button" class="btn btn-info  btn-lm" id="SubmitT" value="<?php //echo $lang_survey_previewpage; ?>" onClick="window.open('survey_p.php?s_id=<?php echo $s_id; ?>','preview','width=500,height=500,resizable=1,status=0,scrollbars=1,location=0,top=40,left=200');">-->
        ...
        <button type="submit" class="btn btn-warning btn-lm" data-toggle="tooltip" data-placement="top" title="<?=$lang_survey_update; ?>"  >
		<span class="glyphicon glyphicon-pencil"></span>&nbsp;<?=$lang_survey_update; ?>
		</button> 
		
		<input name="aal" type="hidden" id="aal2" value="<?=$i; ?>">
        
		<!--<input type="submit" name="SubmitY" value="<?php //echo $lang_survey_save; ?>" class="btn btn-success  btn-lm" >-->
