...
<div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">แบบสอบถาม</h4>
        </div>
...
?>
<div class="col-md-12 col-sm-12 col-xs-12 text-left" > 
<h4>ส่วนที่ <?=$PartName1.$R['c_d']." : ";?><?php if($R['c_name'] !=""){ echo  strip_tags($R['c_name']); }  ?></h4>
...
<tr class="info">
					<?php
					$SQL2 = $db->query("SELECT DISTINCT(p_ans.a_name) 
					...
					?>		
					<th class ="text-center"><?=$Q['a_name'];?></td>
					<?php } ?>	
					</tr>
</thead>					
...
echo '<div class="col-md-12 col-sm-12 col-xs-12" > ';
				while($X = $db->db_fetch_array($SSS)){ 
					...
					if($X['q_anstype']=="D"){ 
						if($RrRows = $db->db_num_rows($SSS1)){
							...
							if($Z['a_other']=="S"){
								...
								echo '<textarea name="ans'.$X['q_id'].'" id="ck'.$k.'" class="form-control margintop10" placeholder="'.$txt.'" type="area"'; 
								if($Z['option4'] != ""){ 
								echo " cols=\"$Z[option4]\" ";
								}  
								if($Z['option3'] != ""){ 
					...
					?>
						...
						<select name="addr_prov<?php echo $X[q_id];?>" id="addr_prov<?php echo $X[q_id];?>"  class="margintop10 form-control s1<?=$k?>" 
						onchange="getAmp(this,'<?php echo $X[q_id];?>')">
						...
						<div class=" marginbottom15">
							<select name="addr_amp<?php echo $X[q_id];?>"  id="addr_amp<?php echo $X[q_id];?>" class="margintop10 form-control s2<?=$k?>"
									onClick="getAmp_null('<?php echo $X[q_id];?>');" onchange="getTam(this,'<?php echo $X[q_id];?>')">
									<option value="">- เลือกอำเภอ -</option>
							 </select>                                              
						</div>
						
						<div class=" marginbottom15">
							<select name="addr_tamb<?php echo $X[q_id];?>"  id="addr_tamb<?php echo $X[q_id];?>"  class="margintop10 form-control s3<?=$k?>"
							onClick="getTam_null('<?php echo $X[q_id];?>');">
					...
					$p=0;
						while($Z = $db->db_fetch_array($SSS1)){
							...
							echo '<div class="radio" style="text-align:left;"><label>';
