...
/*
$sql_supadmin = $db->query_db("SELECT p_id FROM permission   WHERE p_type = 'U' AND 
													pu_id = '".$_SESSION["EWT_SMID"]."' AND 
													UID = '".$_SESSION["EWT_SUID"]."' AND 
													((s_type = '".$ptype."' AND s_permission = '".$ppms."'  AND s_id = '0' ) OR (s_type = 'suser'  )) ",$EWT_DB_USER);
...
<table width="100%" border="0">
  <tr>
    <td class="ewtsubmenu"><strong>My Menu </strong></td>
  </tr>
<?php
...
$f = 0;
 ...
 $sql_menu="SELECT * FROM  menu_list";
 $query_menu = $db->query_db($sql_menu,$EWT_DB_NAME);
 $count_menu = $db->db_num_rows($query_menu); 
...
?>
<?php if($db->check_permission($ptype,"w","")){ 	?>
  <tr>
    <td><img src="../images/folder_closed.gif" width="16" height="16" border="0" align="absmiddle">&nbsp; <input type="checkbox" name="chk0" id="chk<?php echo $i; ?>" value="0"  <?php echo $check;  ?> onClick="chkdis(this,'0',0,<?php echo  $count_menu; ?>);">
        <strong>All Menu</strong></td>
  </tr>
  <?php }?>
    <?php
$i=1;
	//for($y=0;$y<$numfolder;$y++){
	while($rs_cp = $db->db_fetch_array($query_menu)){
			//if($folder != ""){
			...
			//$preview_path_en = base64_encode($preview_path);
			 ...
			 $sql_sadmin = $db->query_db("SELECT * FROM permission1 WHERE p_type = '$mtype' AND pu_id = '$mid' AND UID = '$UID' AND s_type = '".$ptype."' AND s_permission = '".$ppms."'  AND s_id = '0'  AND s_name = '".$rs_cp[m_id]."' AND myFlag = '$myFlag' ",$EWT_DB_USER); 
  	if($db->db_num_rows($sql_sadmin) > 0){
		$check = "checked";
	}else{
		$check = "";
