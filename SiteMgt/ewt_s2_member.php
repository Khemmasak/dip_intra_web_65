...
if($_POST["Flag"] == "Add"){
	for($i=0;$i<$_POST["alli"];$i++){
		...
		$utype = $_POST["utype".$i];
		if($chk == "Y"){
		$sqlchk = $db->query("SELECT COUNT(ugm_id) FROM web_group_member WHERE  ug_id = '".$_POST["ug"]."' AND ugm_type = '".$utype."' AND ugm_tid = '".$uid."' ");
 		 $C = $db->db_fetch_row($sqlchk);
		 		if($C[0] == 0){
						$db->query("INSERT INTO web_group_member ( ug_id,ugm_type,ugm_tid) VALUES ('".$_POST["ug"]."', '".$utype."','".$uid."') ");
				}
		}else{
				$sqlchk = $db->query("DELETE FROM web_group_member WHERE ug_id = '".$_POST["ug"]."' AND ugm_type = '".$utype."' AND ugm_tid = '".$uid."' ");
		}
	}
	?>
...
<form name="form1" method="post" target="_top" action="ewt_s2_member.php">
...
<tr> 
  ...
  ?>
        <div>
      <?php
	  $sqlchk = $db->query("SELECT COUNT(ugm_id) FROM web_group_member WHERE  ugm_type = 'D' AND ugm_tid = '".$R["org_id"]."' ");
 		 $C = $db->db_fetch_row($sqlchk);
	   ...
	   $k++;
			   $sql_position = $db->query("SELECT * FROM position_name INNER JOIN user_position ON position_name.pos_id = user_position.pos_id WHERE user_position.org_id = '".$R["org_id"]."' ORDER BY user_position.up_rank ASC");
				echo "<div id=\"dp".$i."\"  style=\"display:none\">";
					...
					$sqlchk = $db->query("SELECT COUNT(ugm_id) FROM web_group_member WHERE  ugm_type = 'P' AND ugm_tid = '".$P["up_id"]."' ");
 		 $C = $db->db_fetch_row($sqlchk);
						GenPic($R["parent_org_id"]);
						?>
