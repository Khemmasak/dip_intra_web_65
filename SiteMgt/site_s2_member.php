...
if($_POST["Flag"] == "Add"){
	for($i=0;$i<$_POST["alli"];$i++){
		...
		$utype = $_POST["utype".$i];
		if($chk == "Y"){
		$sqlchk = $db->query("SELECT COUNT(ugm_id) FROM web_group_member WHERE   ugm_type = '".$utype."' AND ugm_tid = '".$uid."' ");
 		 $C = $db->db_fetch_row($sqlchk);
		 		if($C[0] == 0){
						$db->query("INSERT INTO web_group_member ( ugm_type,ugm_tid) VALUES ( '".$utype."','".$uid."') ");
				}
		}else{
				$sqlchk = $db->query("DELETE FROM web_group_member WHERE  ugm_type = '".$utype."' AND ugm_tid = '".$uid."' ");
		}
	}
	?>
...
<tr> 
  ...
  $LenChk =0;
	...
	$sql_sub = $db->query_db("SELECT COUNT(org_id) FROM org_name WHERE parent_org_id LIKE '".$R["parent_org_id"]."_%'",$EWT_DB_USER);
  ...
  ?>
        <div>
      <?php
	  $sqlchk = $db->query_db("SELECT COUNT(ugm_id) FROM web_group_member WHERE  ugm_type = 'D' AND ugm_tid = '".$R["org_id"]."' ",$_SESSION["EWT_SDB"]);
 		 $C = $db->db_fetch_row($sqlchk);
	   ...
	   <?php
	   $k++;
			   $sql_position = $db->query_db("SELECT * FROM position_name INNER JOIN user_position ON position_name.pos_id = user_position.pos_id WHERE user_position.org_id = '".$R["org_id"]."' ORDER BY user_position.up_rank ASC",$EWT_DB_USER);
				echo "<div id=\"dp".$i."\"  style=\"display:none\">";
					while($P = $db->db_fetch_array($sql_position)){
					$sqlchk = $db->query_db("SELECT COUNT(ugm_id) FROM web_group_member WHERE  ugm_type = 'P' AND ugm_tid = '".$P["up_id"]."' ",$_SESSION["EWT_SDB"]);
 		 $C = $db->db_fetch_row($sqlchk);
						GenPic($R["parent_org_id"]);
						?>
