...
if($_POST["Flag"] == "Set"){
		...
		$pu_id = $_POST["pu_id".$i];
			...
			if($artw == "Y"){
				$sql_p = $db->query("SELECT COUNT(p_id) FROM permission WHERE p_type = '".$p_type."' AND pu_id = '".$pu_id."' AND s_type = 'art' AND s_permission = 'w' ");
	 			$C = $db->db_fetch_row($sql_p);
	  			if($C[0] == 0){
					$db->query("INSERT INTO permission (p_type,pu_id,s_type,s_id,s_name,s_permission) VALUES ('".$p_type."','".$pu_id."','art','','','w') ");
					}
			}else{
			$db->query("DELETE FROM permission WHERE p_type = '".$p_type."' AND pu_id = '".$pu_id."' AND s_type = 'art' AND s_permission = 'w' ");
			}
			
			if($arta == "Y"){
				$sql_p = $db->query("SELECT COUNT(p_id) FROM permission WHERE p_type = '".$p_type."' AND pu_id = '".$pu_id."' AND s_type = 'art' AND s_permission = 'a' ");
	 			$C = $db->db_fetch_row($sql_p);
	  			if($C[0] == 0){
					$db->query("INSERT INTO permission (p_type,pu_id,s_type,s_id,s_name,s_permission) VALUES ('".$p_type."','".$pu_id."','art','','','a') ");
					}
			}else{
			$db->query("DELETE FROM permission WHERE p_type = '".$p_type."' AND pu_id = '".$pu_id."' AND s_type = 'art' AND s_permission = 'a' ");
			}
			...
			if($guse == "Y"){
				$sql_p = $db->query("SELECT COUNT(p_id) FROM permission WHERE p_type = '".$p_type."' AND pu_id = '".$pu_id."' AND s_type = 'Ag' AND s_id = '$gid' ");
	 			$C = $db->db_fetch_row($sql_p);
	  			if($C[0] == 0){
					$db->query("INSERT INTO permission (p_type,pu_id,s_type,s_id,s_name,s_permission) VALUES ('".$p_type."','".$pu_id."','Ag','$gid','','') ");
					}
			}else{
			$db->query("DELETE FROM permission WHERE p_type = '".$p_type."' AND pu_id = '".$pu_id."' AND s_type = 'Ag' AND s_id = '$gid' ");
			}
			}
		...
		self.location.href = "p_article.php";
		</script>
		<?php
		exit;
}
...
<body><form name="form1" method="post" action="p_article.php">
  <table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#CCCCCC">
    <tr bgcolor="E0DFE3"> 
	  ...
	  <?php
	    $sql_group = $db->query("SELECT * FROM article_group  ");
		$rows = $db->db_num_rows($sql_group);
	  ?>
      <td colspan="<?php echo $rows; ?>" align="center" class="head_table">Group 
        permission</td>
    </tr>
    <tr bgcolor="E0DFE3"> 
      <?php 
	  ...
	  $gid[$k] = $GR["c_id"];
	   ?>
      <td width="5%" align="center" class="head_table"><?php echo $GR["c_name"]; ?></td>
      <?php $k++; }  ?>
    </tr>
    <?php $i = 0; ?>
    <?php
 $i=0;
  ...
  ?>
    <tr bgcolor="#FFFFFF"> 
      ...
      <?php
	  $sql_p = $db->query_db("SELECT COUNT(p_id) FROM permission WHERE p_type = '".$U["ugm_type"]."' AND pu_id = '".$U["ugm_tid"]."' AND s_type = 'art' AND s_permission = 'w' ",$_SESSION["EWT_SDB"]);
	  $C = $db->db_fetch_row($sql_p);
	  ?>
      <td align="center" bgcolor="#E7E7E7"><input type="checkbox" name="artw<?php echo $i; ?>" value="Y" <?php if($C[0] > 0){ echo "checked"; } ?>></td>
	  <?php
	  $sql_p = $db->query_db("SELECT COUNT(p_id) FROM permission WHERE p_type = '".$U["ugm_type"]."' AND pu_id = '".$U["ugm_tid"]."' AND s_type = 'art' AND s_permission = 'a' ",$_SESSION["EWT_SDB"]);
	  $C = $db->db_fetch_row($sql_p);
	  ?>
      <td align="center" bgcolor="#E7E7E7"><input type="checkbox" name="arta<?php echo $i; ?>" value="Y" <?php if($C[0] > 0){ echo "checked"; } ?>></td>
	  <?php for($x=0;$x<$rows;$x++){ 
	  $sql_p = $db->query_db("SELECT COUNT(p_id) FROM permission WHERE p_type = '".$U["ugm_type"]."' AND pu_id = '".$U["ugm_tid"]."' AND s_type = 'Ag' AND s_id = '$gid[$x]' ",$_SESSION["EWT_SDB"]);
	  $C = $db->db_fetch_row($sql_p);
	  ?>
