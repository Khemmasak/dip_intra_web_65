...
//"SELECT p_id FROM permission WHERE p_type = 'U' AND pu_id = '".$_SESSION["EWT_SMID"]."' AND UID = '".$_SESSION["EWT_SUID"]."' AND ((s_type = '".$ptype."' AND s_permission = '".$ppms."'  AND s_id = '0' ) OR (s_type = 'suser'  )) ";
//echo "SELECT p_id FROM permission WHERE p_type = 'U' AND UID = '".$_SESSION["EWT_SUID"]."' AND ((s_type = '".$ptype."' AND s_permission = '".$ppms."'  AND s_id = '0' ) OR (s_type = 'suser'  )) ".$EWT_DB_USER;
...
<tr>
    <td class="ewtsubmenu">หมายเหตุ: การกำหนดสิทธิ์ของ My Download ท่านสามารถกำหนดได้ใน folder ระดับที่ 1 เท่านั้น</td>
  </tr>
  <tr>
    <td class="ewtsubmenu">&nbsp;</td>
  </tr>
  <tr>
    <td class="ewtsubmenu"><strong>My Download </strong></td>
  </tr>
<?php
...
?>
<?php if($db->check_permission("dlFo","w","")){ 
	?>
  <tr>
...
$i=1;
	...
	?>
		<?php if($db->check_permission("dlFo","w","".$array_folder[$y]."")){ 
	?>
  <tr>
