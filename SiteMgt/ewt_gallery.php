...
if($_SESSION["EWT_SMID"] != ''){
	...
	window.location.href ="ewt_gallery2.php?mid=<?php echo $_GET["mid"]; ?>&mtype=<?php echo $_GET["mtype"]; ?>&myFlag=<?php echo $_GET["myFlag"]; ?>";
	</script>
	<?php
	exit;
	}
}
$sql = $db->query_db("SELECT * FROM gallery_category WHERE parent_id = '0' ORDER BY category_id ASC ",$_SESSION["EWT_SDB"]);
