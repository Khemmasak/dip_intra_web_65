...
$sql = $db->query_db("SELECT * FROM cal_category WHERE CHAR_LENGTH(parent_cat_id) = 4 ORDER BY cat_id ASC ",$_SESSION["EWT_SDB"]);
$rTotal = $db->db_fetch_array($db->query('SELECT COUNT(cat_id) AS totalCat FROM cal_category'));
$totalCat=$rTotal['totalCat'];
...
$y = $x+1;
$sql = $db->query_db("SELECT * FROM cal_category WHERE parent_cat_id LIKE '".$c."_%' AND CHAR_LENGTH(parent_cat_id)-CHAR_LENGTH('$c')=5",$_SESSION["EWT_SDB"]);
  while($U = $db->db_fetch_array($sql)){
  $sql_sadmin = $db->query_db("SELECT * FROM permission1 WHERE p_type = '$mtype' AND pu_id = '$mid' AND UID = '$UID' AND s_type = '".$ptype."' AND s_permission = '".$ppms."'  AND s_id = '".$U["cat_id"]."' AND myFlag = '$myFlag' ",$EWT_DB_USER);
  	if($db->db_num_rows($sql_sadmin) > 0){
		$check = "checked";
	}else{
		$check = "";
	}
	$ct = countchild($U["parent_cat_id"]);
  ?>
    <tr bgcolor="#FFFFFF" > 
	  ...
	  &nbsp;<input type="checkbox" name="chk<?php echo $i;  ?>" id="chk<?php echo $i;  ?>" onClick="chkdis(this,'<?php echo $U["cat_id"]; ?>',<?php echo $i; ?>,<?php echo $ct; ?>);" value="<?php echo $U["cat_id"]; ?>" <?php echo $decho;  ?> <?php echo $check;  ?>>
	  <?php echo $U["cat_name"]; ?></td>
    </tr>
    <?php
	if($check == "checked"){
	$txt .= "chkdis(document.form1.chk".$i.",'".$U["cat_id"]."',".$i.",".$ct."); ";
	}
	$i++; 
	child($U["parent_cat_id"],$y,$decho);
  }
}
...
function updates(c,gid){
	if(c.checked == true){
	var	url='ewt_upd_ajax.php?UID=<?php echo $UID; ?>&mid=<?php echo $mid; ?>&mtype=<?php echo $mtype; ?>&myFlag=<?php echo $myFlag; ?>&accept=Y&ptype=<?php echo $ptype; ?>&ppms=<?php echo $ppms; ?>&psid=' + gid + "&psname=" + gid;
	}else{
	var	url='ewt_upd_ajax.php?UID=<?php echo $UID; ?>&mid=<?php echo $mid; ?>&mtype=<?php echo $mtype; ?>&myFlag=<?php echo $myFlag; ?>&accept=N&ptype=<?php echo $ptype; ?>&ppms=<?php echo $ppms; ?>&psid=' + gid + "&psname=" + gid;
	}
	AjaxRequest.get(
		{
			'url':url
...
<span id="divhtml" style="display:none"></span>
  <table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#CCCCCC">
	...
	<tr bgcolor="#FFFFFF" > 
      <td ><strong>หมวดกิจกรรม</strong></td>
    </tr>
	...
	?>
      <td ><img src="../images/folder_closed.gif" width="16" height="16" border="0" align="absmiddle">&nbsp; <input type="checkbox" name="chk0" id="chk0" value="0"  <?php echo $check;  ?> onClick="chkdis(this,'0',0,<?php echo $totalCat;  ?>);">
        <strong>ทุกหมวด</strong></td>
    </tr>
    <?php
	$i=1;
	$txt = "";
