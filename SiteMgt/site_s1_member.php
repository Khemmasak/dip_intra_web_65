...
if($_POST["Flag"] == "Add"){
	for($i=0;$i<$_POST["alli"];$i++){
		...
		$uid = $_POST["uid".$i];
		if($chk == "Y"){
		$sqlchk = $db->query("SELECT COUNT(ugm_id) FROM web_group_member WHERE ugm_type = 'U' AND ugm_tid = '".$uid."' ");
 		 $C = $db->db_fetch_row($sqlchk);
		 		if($C[0] == 0){
						$db->query("INSERT INTO web_group_member (ugm_type,ugm_tid) VALUES ('U','".$uid."') ");
				}
		}else{
				$sqlchk = $db->query("DELETE FROM web_group_member WHERE ugm_type = 'U' AND ugm_tid = '".$uid."' ");
		}
	}
	?>
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#CCCCCC"><form name="form1" method="post" target="_top" action="site_s1_member.php">
  ...
  $sqlchk = $db->query_db("SELECT COUNT(ugm_id) FROM web_group_member WHERE ugm_type = 'U' AND ugm_tid = '".$U["gen_user_id"]."' ",$_SESSION["EWT_SDB"]);
  $C = $db->db_fetch_row($sqlchk);
  ?>
  <tr bgcolor="#FFFFFF"> 
