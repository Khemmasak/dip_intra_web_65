...
$sql = $db->query_db("SELECT * FROM article_group ORDER BY c_id ASC ",$R["db_db"]);
...
<span id="divhtml" style="display:none"></span>
 ...
 $i=0;
  ...
  $sql_sadmin = $db->query_db("SELECT * FROM permission2 WHERE p_type = '$mtype' AND pu_id = '$mid' AND UID = '$UID' AND s_type = '".$ptype."' AND s_permission = '".$ppms."'  AND s_id = '".$U["c_id"]."'  AND myFlag = '".$_GET["myFlag"]."' ",$EWT_DB_USER);
