...
//check username in db
	function check_name_wbb($name){
		...
		$db->query("USE ".$EWT_DB_USER);
		$sql_img = "select * from gen_user where webb_name = '".$name."' and gen_user_id <> '".$_SESSION["EWT_SMID"]."'";
		$query = $db->query($sql_img);
		if($db->db_num_rows($query)>0){
			print "<script>alert('ชื่อนามแฝงนี้มีสมาชิกของระบบใช้อยู่ กรุณาใช้ชื่อนามแฝงอื่น!!!!!!!!!".$_SESSION["EWT_SMID"]."');</script>";	exit;
		}else{
		return $name;
		}
...
$id=$_SESSION["EWT_SMID"];
			  ...
			  if($row_emp > 0){  
			   ?>
				 <script language="javascript">
				  ...
				  document.location.href="frm_edit_member.php";
				  </script>
	  	 	<?php
		   	 		exit;
			  }else{
	...
	$update="Update gen_user Set  emp_id='".$emp_id."',
														  ...
														  last_update=NOW(),
														  last_update_by='".$_SESSION["EWT_SUSER"]."',
														  expiredate='' ,
														  gen_user = '".$user_name."',
		...
		echo "self.parent.location.href='index_member_out2.php'" ;
		echo "</script>";	
}
	...
	$path_image= $uploaddir.$rst_main[path_image];
						if (file_exists($path_image)) {
					   $path_image=$path_image;
					    $hfile=$rst_main[path_image];
					   }else{
	...
	$path_image2 = "../ewt/pic_upload_webboard/".$rst_main[webb_pic];
	if (file_exists($path_image2)) {
					   $path_image2=$path_image2;
					   $hfile2=$rst_main[webb_pic];
					   }else{
   ...
   <?php if($emp_type_status!='2'){?>
	...
	if(frm.chk_edit_login.checked == true){
		...
		return false;
			...
			if (frm.old_password.value.search("^[A-Za-z0-9_]+$")){
...
<form name="frm"  action=""  method="post"  enctype="multipart/form-data">
  <table width="80%" border="0" align="center" cellpadding="5" cellspacing="0">
    <tr>
      <td><img src="../theme/main_theme/logo.gif" width="32" height="32" align="absmiddle"> <span class="ewtfunction">Edit Profile </span> </td>
    </tr>
  </table>
...
<input type="hidden" name="hfile" value="<?php echo $hfile;?>" />
                <br/><br/><?php } ?>
        ...
        <script language="javascript">
            ...
            <tr>
              ...
              <td align="center"><img src="img.php?p=<?php echo base64_encode($path_image2); ?>" name="previewField2" width="120"    id="previewField2" />
			    <input type="hidden" name="hfile2" value="<?php echo $hfile2;?>" />
