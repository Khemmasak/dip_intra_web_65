...
// add category
	if($_POST["flag"]=='add'){
		...
		$db->query("INSERT INTO `f_cat` ( `f_id` , `f_cate` , `f_detail`, `f_use` ,`f_no`,`f_date`,`f_time`) VALUES ('', '$t_topic', '$t_detail','Y','$f_no',NOW(),NOW())");
...
//edit category
	...
	$t_detail = eregi_replace(chr(13)," <br> ", $t_detail );
	$db->query("UPDATE   f_cat SET f_cate = '$t_topic', f_detail = '$t_detail' ,f_no='$f_no'   WHERE f_id = '".$_POST["f_id"]."'");
	...
	if($_POST["flag"]=='add'){
			...
			$t_detail = addslashes(htmlspecialchars($_POST["t_detail"]));
			$t_detail = eregi_replace(chr(13)," <br> ", $t_detail );
			$db->query("INSERT INTO `f_subcat` ( `f_id` , `f_subcate` , `f_subdetail`, `f_use` ,`f_sub_no`,`f_sub_date`,`f_sub_time`) VALUES ('".$_POST["f_id"]."', '$t_topic', '$t_detail','Y','$f_sub_no',NOW(),NOW())");
	...
	if($_POST["flag"] == "edit"){
			...
			$t_detail = addslashes(htmlspecialchars($t_detail));
			$t_detail = eregi_replace(chr(13)," <br> ", $t_detail );
			$db->query("UPDATE   f_subcat SET f_subcate = '$t_topic', f_subdetail = '$t_detail' , f_id = '".$_POST["f_id"]."',f_sub_no='".$_POST["f_sub_no"]."'  WHERE f_sub_id = '".$_POST["f_sub_id"]."'   ");
...
$db->query("INSERT INTO faq (fa_id,f_id,fa_name,fa_detail,fa_ans,f_sub_id,faq_use,faq_top,faq_date) VALUES ('','".$_POST["f_id"]."','$fname','$fdetail','$fans','".$_POST["f_sub_id"]."','".$_POST["faq_use"]."','".$_POST["faq_top"]."','$today')");
...
window.location.href = "faq_question.php?f_id=<?php echo $_POST["f_id"]; ?>&f_sub_id=<?php echo $_POST["f_sub_id"]; ?>";
</script>
	<?php
}
...
$db->query("UPDATE faq SET fa_name = '$fname',fa_detail = '$fdetail',fa_ans = '$fans'  ,faq_use = '".$_POST["faq_use"]."',faq_top = '".$_POST["faq_top"]."',faq_date= '$today',f_sub_id='".$_POST["faqsub_id"]."'     WHERE fa_id = '".$_POST["fa_id"]."'");
...
if($_POST["flag"] == "approve"){
	for($i=0;$i<count($_POST["user_id"]);$i++){
		...
		$rec = $db->db_fetch_array($db->query( "select * from faq_user where faq_user_id = '$chk'"));
			if($srt == '1'){
				$db->query("INSERT INTO faq (fa_id,f_id,fa_name,fa_detail,fa_ans,f_sub_id,faq_use,faq_top,faq_date) VALUES ('','".$rec["f_id"]."','".$rec["faq_user_name"]."','".$rec["faq_user_detail"]."','".$rec["faq_user_ans"]."','".$rec["f_sub_id"]."','Y','','$today')");
