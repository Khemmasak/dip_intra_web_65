...
$pdf->str_date=$date_name;
  ...
  $sql=$db->query("SELECT faq.fa_id,  faq.fa_name,count(faq_answer.faq_user_id) as max  FROM faq LEFT JOIN faq_answer ON faq.fa_id = faq_answer.fa_id   $con WHERE f_sub_id = '".$_REQUEST[fid]."'  GROUP BY faq.fa_id, faq.fa_name ORDER BY max DESC");
...
if($db->db_num_rows($sql)>0){
			while($R=$db->db_fetch_array($sql)){
			   ...
			   $pdf->Row(array($i,$R[fa_name],$R["max"]));
			   $strDif="";
			   $i++;
			}
}else{
