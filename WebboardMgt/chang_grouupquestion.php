...
if($flag=='chang'){
$rec = $db->db_fetch_array($db->query("select * from w_question WHERE t_id = '$wtid'"));
$rec2 = $db->db_fetch_array($db->query("select * from w_cate WHERE c_id = '$wcad_new'"));
$db->write_log("chang","webboard","จัดหมวดหมู่คำถาม  ".$rec[t_name]." ไปอยู่ในหมวด   ".$rec2[c_name]);
