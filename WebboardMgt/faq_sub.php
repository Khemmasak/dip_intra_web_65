<?php
include("administrator.php");
include("lib/include.php");
include("inc.php");
include("../language.php");

function current_faq_level($fid){
	global $db;
	$sql = "select * from f_subcat where f_sub_id = '$fid' ";
	$query = $db->query($sql); 
	$R = mysql_fetch_array($query);
	if($R[f_parent] > 0){  current_faq_level($R[f_parent]); }
	if($fid<>0){
		 echo ' > <a href="faq_sub.php?f_id='.$R[f_sub_id].'">'.$R[f_subcate].'</a>';
	}
}
function current_faq_level2($fid){
	global $db;
	$sql = "select * from f_subcat where f_sub_id = '$fid' ";
	$query = $db->query($sql); 
	$R = mysql_fetch_array($query);
	if($R[f_parent] > 0){  current_faq_level2($R[f_parent]); }
	if($fid<>0){
		 return '>'.$R[f_subcate];
	}
}
function current_sub_count($fid){
	global $db;
	 $sql = "SELECT f_sub_id FROM f_subcat WHERE f_parent = '$fid' ";
	 $query = $db->query($sql);
	 $count = mysql_num_rows($query); 
	 if($count>0){
		 while($data=$db->db_fetch_array($query)){
		    $count=$count + current_sub_count($data[f_sub_id]);
		 }
	}
	return $count;
}

function current_faq_count($fid){
	global $db;
	 $sql = "SELECT f_sub_id FROM f_subcat WHERE f_parent = '$fid' ";
	 $query = $db->query($sql);
	 $count = mysql_num_rows($query);
	 
	 $sql2 = "SELECT fa_id FROM faq WHERE f_sub_id = '$fid'  ";
	 $query2 = $db->query($sql2);
	 $count2 = mysql_num_rows($query2); 
	 if($count>0){
		 while($data=$db->db_fetch_array($query)){
		    $count2=$count2 + current_faq_count($data[f_sub_id]);
		 }
	}
	return $count2;
}

$i=0;
if($_GET["f_id"]){
    $Execsql = $db->query("SELECT * FROM f_subcat WHERE  f_sub_id = '".$_GET["f_id"]."' ");
	$R = mysql_fetch_array($Execsql);
	$f_sub_id=$R[f_parent];
	$f_id=$R[f_sub_id];

    $Execsql = $db->query("SELECT * FROM f_subcat WHERE  f_parent = '".$_GET["f_id"]."' ");
	$R = mysql_fetch_array($Execsql);
}else{
	$Execsql = $db->query("SELECT * FROM f_subcat WHERE f_parent='0' ");
}
?>
<html>
<head>
<title><?php echo $EWT_title ?></title>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<link href="../theme/main_theme/css/style.css" rel="stylesheet" type="text/css">
<script language="JavaScript">
	function DelForm(){
		if(confirm("คุณแน่ใจที่จะลบข้อมูล?")){
			form1.submit();
		}
	}
	
	function DelForm2(){
		if(confirm("คุณแน่ใจที่จะลบข้อมูล?")){
			form2.submit();
		}
	}
</script>
</head>
<body leftmargin="0" topmargin="0">
 <?php include("../FavoritesMgt/favorites_include.php");?>
<table width="80%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr> 
    <td><img src="../theme/main_theme/fag_function.gif" width="32" height="32" align="absmiddle"> <span class="ewtfunction">
   <a href="faq_sub.php"><?php echo $text_genfaq_category;?></a><?php current_faq_level($_GET["f_id"]);?></span></td>
  </tr>
</table>
<table width="94%" border="0" align="center" cellpadding="0" cellspacing="0" class="ewtfunctionmenu">
  <tr>
    <td align="right"><a href="javascript:void(0);" onClick="load_divForm('../FavoritesMgt/favorites_add.php?name=<?php echo urlencode($text_genfaq_category.current_faq_level2($_GET["f_id"]));?>&module=faq&url=<?php echo urlencode("faq_sub.php?f_id=".$_GET["f_id"]);?>', 'divForm', 300, 80, -1,433, 1);"><img src="../images/star_yellow_add.gif" width="16" height="16" border="0" align="middle">&nbsp;Add to favorites </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="faq_add.php?type=category_sub&flag=add&f_id=<?php echo $_GET["f_id"]?>"><img border="0" src="../theme/main_theme/g_add.gif" width="16" height="16" align="absmiddle" alt=" <?php echo $text_general_add;?>"> 
      <?php echo $text_general_add;?></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="faq_sub.php?f_id=<?php echo $f_sub_id;?>"><img src="../theme/main_theme/g_back.gif" alt="กลับ" width="16" height="16" border="0" align="absmiddle"> 
     <?php echo $text_general_back;?></a>
      <hr>
    </td>
  </tr>
</table>

<table width="94%" border="0" cellspacing="1" cellpadding="3" align="center">
				 <form method="post" action="faq_sub.php?f_id=<?php echo $_GET["f_id"]?>">
								<tr>
								  <td ><input type="hidden" name="curPage" value="1">
									ค้นหาหมวด
									<input type="text" name="data" value="<?php echo $data;?>">
								  <input type="submit" name="Submit" value="ค้นหา"></td>
								</tr>
				</form>	
</table>
<table width="94%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CECECE" class="ewttableuse">
 <form name="form1" method="post" action="faqfunction.php">
  <input type="hidden" name="flag" value="delete">
  <input type="hidden" name="f_id" value="<?php echo $_GET["f_id"]?>">
   <input type="hidden" name="type" value="category_sub">
  <tr bgcolor="#E7E7E7"  class="ewttablehead"> 
    <td width="10%" height="47" align="center">&nbsp;</td>
    <td width="55%" ><?php echo $text_genfaq_categorysub;?></td>
    <td width="15%" ><?php echo $text_genfaq_subgorycount;?></td>
    <td width="15%" ><?php echo $text_genfaq_categorycount;?></td>
    <td width="5%" align="center"><?php echo $text_general_delete;?></td>
  </tr>
<?php  

 $data = $_REQUEST['data'];
 $f_id=$_GET[f_id]*1;
 
 if (!empty($data)) {
		$wh = " AND f_subcate like '%$data%' ";
}
$sql_subcat="select * from f_subcat  where f_parent='$f_id'  $wh order by  f_sub_no  "  ;

					$query_subcat=$db->query($sql_subcat);
					 if(mysql_num_rows($query_subcat) > 0){
					while($R_SUB=$db->db_fetch_array($query_subcat)){

 ?>
  <tr bgcolor="#FFFFFF"> 
    <td align="center"><nobr><a href="#" onClick="window.location.href='faq_add.php?type=category_sub&flag=edit&f_sub_id=<?php echo $R_SUB[f_sub_id];?>&f_id=<?php echo $_GET[f_id]?>'"><img border="0" src="../theme/main_theme/g_edit.gif" width="16" height="16" alt="<?php echo $text_general_edit;?>"> </a>
				  <?php if($R_SUB[f_use]=='Y') {?>
				  <a href="faqfunction.php?type=category_sub&flag=hide&f_sub_id=<?php echo $R_SUB[f_sub_id];?>&f_id=<?php echo $_GET[f_id]?>" onClick="return confirm('Are you sure to drop this subcategory?');"><img src="../theme/main_theme/g_hide.gif" alt="<?php echo $text_general_hide;?>" width="16" height="16" border="0"></a>
				  <?php }else{  ?>
				  <a href="faqfunction.php?type=category_sub&flag=show&f_sub_id=<?php echo $R_SUB[f_sub_id];?>&f_id=<?php echo $_GET[f_id]?>" onClick="return confirm('Are you sure to show this subcategory?');"><img src="../theme/main_theme/g_show.gif" alt="<?php echo $text_general_enable;?>" width="16" height="16" border="0"></a>
				  <?php }?>
				  <a href="addfaq.php?f_sub_id=<?php echo $R_SUB[f_sub_id];?>&f_id=<?php echo $_GET["f_id"];?>"><img src="../theme/main_theme/faq_categoryadd_sub.gif" width="16" height="16" border="0" align="absmiddle" alt="<?php echo $text_genfaq_faqadd;?>"></a></nobr></td>
    <td><a href="faq_sub.php?f_id=<?php echo $R_SUB[f_sub_id]; ?>"><?php  biz($R_SUB[f_subcate]); ?></a></td>
    <td align="center"><?php
   $count1 = $db->query("SELECT f_sub_id FROM f_subcat WHERE f_parent = '$R_SUB[f_sub_id]'  ");
  if(mysql_num_rows($count1)){  echo mysql_num_rows($count1);  }else{ echo '0';  }
  if(current_sub_count($R_SUB[f_sub_id])>0){  echo ' of  '.current_sub_count($R_SUB[f_sub_id]);  }
  
?></td>
    <td align="center">
	<a href="faq_question.php?f_id=<?php echo $R[f_parent]; ?>&f_sub_id=<?php echo $R_SUB[f_sub_id]; ?>">
	<?php
	//echo "SELECT fa_id FROM faq WHERE f_sub_id = '$R_SUB[f_sub_id]'  ";
   $count2 = $db->query("SELECT fa_id FROM faq WHERE f_sub_id = '$R_SUB[f_sub_id]'  ");
    if(mysql_num_rows($count2)){  echo mysql_num_rows($count2);  }else{ echo '0';  }
    if(current_faq_count($R_SUB[f_sub_id])>0){  echo ' of  '.current_faq_count($R_SUB[f_sub_id]);  }
    ?></a>
</td>
    <td align="center"><input name="chk<?php echo $i; ?>" type="checkbox" id="chk<?php echo $i; ?>" value="<?php echo $R_SUB[f_sub_id];?>"></td>
  </tr>
  <?php  $i++; } ?>
  <tr bgcolor="#FFFFFF"> 
    <td colspan="4">&nbsp;</td>
    <td align="center"><input type="button" name="Button" value="<?php echo $text_general_delete;?>" onClick="DelForm()"></td>
  </tr>
  <?php }else{ ?>
    <tr bgcolor="#FFFFFF"> 
    <td colspan="5" align="center"><font color="#FF0000"><?php echo $text_genfaq_categorysub_disabledata;?></font></td>
  </tr>
  <?php } ?>
  <input name="alli" type="hidden" id="alli" value="<?php echo $i; ?>">
  </form>
</table>



<!--<table width="94%" border="0" align="center" cellpadding="3" cellspacing="0">
  <tr>
    <td>Page : &lt;&lt; Pre&nbsp;&nbsp;1&nbsp;&nbsp;<strong>[2]</strong>&nbsp;&nbsp;3&nbsp;&nbsp;4&nbsp;&nbsp;5&nbsp;&nbsp;6&nbsp;&nbsp;Next &gt;&gt;</td>
  </tr>
</table>-->



<?php  if($_GET["f_id"]){ 

$chk_config = $db->query("SELECT * FROM w_config WHERE c_config = '1'");
$CO = mysql_fetch_array($chk_config);
$sel = "SELECT * FROM faq WHERE f_sub_id = '$f_id' ORDER BY  faq_top DESC ,fa_id ASC";

   if (empty($offset) || $offset < 0) { 
        $offset=0; 
    } 
//    Set $limit,  $limit = Max number of results per 'page' 
$limit = $CO[c_number];

//    Set $totalrows = total number of rows that unlimited query would return 
//    (total number of records to display across all pages) 
$ExecSel = $db->query($sel);
$rows = mysql_num_rows($ExecSel);

	// Set $begin and $end to record range of the current page 
    $begin =($offset+1); 
    $end = ($begin+($limit-1)); 
    if ($end > $totalrows) { 
        $end = $totalrows; 
    } 
$Show = $sel;//." LIMIT $offset, $limit ";
$Execsql = $db->query($Show);

?>

<table width="94%" border="0" align="center" cellpadding="5" cellspacing="0">
<?php
$Execsql1 = $db->query("SELECT * FROM f_subcat WHERE  f_sub_id = '$f_id' ");
$R1 = mysql_fetch_array($Execsql1);
$sql_subcat="select * from f_subcat where f_sub_id='$f_sub_id' order by  f_sub_no  "  ;
$query_subcat=$db->query($sql_subcat);
$R_SUB=$db->db_fetch_array($query_subcat);
?>
  <tr> 
    <td align="left"><span class="ewtfunction"> FAQ ในหมวด <?php echo $R1[f_subcate]?></span></td>
  </tr>
</table>
<table width="94%" border="0" align="center" cellpadding="0" cellspacing="0" class="ewtfunctionmenu">
  <tr>
    <td align="right"><a href="addfaq.php?f_sub_id=<?php echo $f_id;?>&f_id=<?php echo $f_sub_id;?>"><img src="../theme/main_theme/g_add.gif" alt="เพิ่ม" width="16" height="16" border="0" align="absmiddle"> 
		<?php echo $text_general_add;?></a>
      <hr>
    </td>
  </tr>
</table>
<table width="94%" border="0" align="center" cellpadding="5" cellspacing="0">
  <!--<tr> 
    <td align="left">&nbsp;<img src="../theme/main_theme/g_search.gif" width="20" height="20" align="absmiddle"> 
      Search 
      <input type="text" name="textfield">
      <input type="submit" name="Submit" value="Search"></td>
  </tr>-->
</table>
<table width="94%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CECECE" class="ewttableuse">
<form name="form2" method="post" action="faqfunction.php">
  <input type="hidden" name="flag" value="delfaq">
  <input type="hidden" name="f_sub_id" value="<?php echo $f_sub_id?>">
  <input type="hidden" name="f_id" value="<?php echo $f_id?>">
  <tr bgcolor="#E7E7E7"  class="ewttablehead"> 
    <td width="5%" height="30" align="center">&nbsp;</td>
    <td ><?php echo $text_genfaq_faq;?></td>
    <td width="5%" align="center"><?php echo $text_general_delete;?></td>
  </tr>
    <?php
	$i = 0;
	if($rows > 0){
   while($R = mysql_fetch_array($Execsql)){ 
	?>
  <tr bgcolor="#FFFFFF"> 
    <td align="center"><a href="addfaq.php?fa_id=<?php echo $R[fa_id]; ?>&f_sub_id=<?php echo $R[f_sub_id]; ?>&f_id=<?php echo $f_id;?>"><img src="../theme/main_theme/g_edit.gif" alt="แก้ไข" width="16" height="16" border="0"></a> 
	<?php 
	$sql_answer = "select count(faq_user_id) as max from faq_answer where fa_id ='".$R[fa_id]."'";
	$query_answer = $db->query($sql_answer);
	$RA = $db->db_fetch_array($query_answer);
	if($RA[max]>'0'){
	?>
	<a href="#" onClick="window.open('faq_view_answer.php?fa_id=<?php echo $R[fa_id]; ?>','','toolbars=no,menubar=no,locatoion=on,scrollbars=yes,width=800,height=500')"><img src="../images/article_newfile.gif" alt="คำถามเพิ่มเติม" width="16" height="16" border="0"></a>
	<?php } ?></td>
    <td><a href="#" onClick="window.open('faq_view.php?fa_id=<?php echo $R[fa_id]; ?>','','toolbars=no,menubar=no,locatoion=on,scrollbars=yes,width=600,height=400')"><?php biz($R[fa_name]); ?></a></td>
    <td align="center"><input name="chk<?php echo $i; ?>" type="checkbox" id="chk<?php echo $i; ?>" value="<?php echo $R[fa_id]; ?>"></td>
  </tr>
  <?php $i++; }?>
  <tr bgcolor="#FFFFFF"> 
    <td colspan="2">&nbsp;</td>
    <td align="center"><input type="button" name="Button" value="<?php echo $text_general_delete;?>" onClick="DelForm2()"></td>
  </tr>
  <?php }else{ ?>
    <tr bgcolor="#FFFFFF"> 
    <td colspan="3" align="center"><font color="#FF0000"><?php echo $text_genfaq_faq_disabledata;?></font></td>
  </tr>
  <?php } ?>
  <input name="alli" type="hidden" id="alli" value="<?php echo $i; ?>">
</form>
</table>

<?php } ?>
</body>
</html>
<?php @$db->db_close(); ?>