...
$db->write_log("view","faq","เข้าหน้า search คำค้น");
$month =date('Y-m').'-%';
$query=$db->query("select count(search_word) as sum_search ,search_word from search_stat  where search_module='5' $wh  group by search_word  order by  sum_search desc $list_limit") ;
...
<table width="94%" border="0" align="center" cellpadding="3" cellspacing="0" class="ewtfunctionmenu">
  <tr> 
    <form name="form2" method="post" action="search_stat_faq.php"  >
      <td> <table width="60%" border="0" align="center" cellpadding="5" cellspacing="1" class="ewttableuse">
         ...
         <tr>
           ...
           <td width="73%" bgcolor="#FFFFFF"><input name="start_date" id="start_date" type="text" size="15" value="<?php echo $_POST[start_date];?>" >
             ...
             ถึง 
             <input name="end_date" id="end_date" type="text" size="15"  value="<?php echo $_POST[end_date];?>">
...
<table  width="94%"  border="0" align="center" cellpadding="5" cellspacing="1" class="ewttableuse">
  <form name="form1" method="post" action="article_function.php">
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td align="center" ><a href="##" onClick="window.open('../search/search_stat_viewdetail.php?search_word=<?php echo $rec[search_word]; ?>&flaglist=faq', 'search_stat_viewdetail', 'status=yes, menubar=no, scrollbars=yes, resizable=yes, height=450, width=600, left=150,top=100');"><img src="../images/bar_view.gif" width="20" height="20" border="0"></a></td>
    </tr>
	<?php } ?>
  </form>
</table>
