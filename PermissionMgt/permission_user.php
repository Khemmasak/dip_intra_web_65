...
<div class="container-fluid">
	...
	if ($_GET["search"] == "Y") {
		...
		if ($fullname != "") {
			...
			foreach ($find as $find_e) {
				if ($find_e != "") {
					$find_e = ready($find_e);
					$where_user .= " AND ((LOWER(g.name_thai) LIKE LOWER('%$find_e%')) OR (LOWER(g.surname_thai) LIKE LOWER('%$find_e%'))) ";
				}
			}
		}
		...
		if ($username != "") {
			...
			foreach ($find as $find_e) {
				if ($find_e != "") {
					$find_e = ready($find_e);
					$where_user .= " AND ((LOWER(g.gen_user) LIKE LOWER('%$find_e%'))) ";
				}
			}
		}
	} else {
	...
	##===========================================================================================##
	$_sql = $db->query("SELECT wgm.* FROM web_group_member wgm WHERE wgm.ug_id = '{$_SESSION['EWT_SUID']}' AND wgm.ugm_tid IN (SELECT gen_user_id FROM gen_user g WHERE $where_user) ORDER BY wgm.ugm_type ASC LIMIT {$start},{$perpage} ");
	$statement = "SELECT count(ugm_id) AS b FROM  web_group_member wgm WHERE wgm.ug_id = '{$_SESSION['EWT_SUID']}' AND wgm.ugm_tid IN (SELECT gen_user_id FROM gen_user g WHERE $where_user)";
	//}
												...
												<div class="panel panel-default ">
													<div class="panel-heading ewt-bg-success">
														<p class="panel-title">
															...
															<img src="<?php //echo $path_image; ?>" alt="" class="img-circle img-rounded " style="width:26px;height:26px;"> -->
															...
															<span class="pull-right">
																<button type="button" class="btn btn-danger  btn-circle  btn-sm" onClick="JQDelete_Permission_user('<?php echo $a_data['ugm_tid']; ?>');" data-toggle="tooltip" data-placement="top" title="<?php echo $txt_permission_del_user; ?>">
																...
																<a href="permission_builder_user.php?mid=<?php echo url_encode($a_data['ugm_tid']); ?>&mtype=<?php echo url_encode($a_data['ugm_type']); ?>">
																	<button type="button" class="btn btn-info  btn-circle  btn-sm " data-toggle="tooltip" data-placement="top" title="<?php echo $txt_permission_set; ?>">
...
function show_permission($type, $id)
{
	...
	if ($db->db_num_rows($sql_sadmin) > 0) {
		...
		while ($PP = $db->db_fetch_row($sql_p)) {
			...
			if (($PP[1] == "img" or $PP[1] == "dl") and $PP[2] == "w") {
				...
				if ($db->db_num_rows($sql_f_all) > 0) {
					...
					for ($y = 0; $y < $numfolder; $y++) {
						...
						if ($PP[1] == "img") {
							$sql_sadmin = $db->query("SELECT * FROM permission WHERE p_type = '" . $type . "' AND pu_id = '" . $id . "' AND UID = '" . $_SESSION["EWT_SUID"] . "' AND s_type = 'imgFo' AND s_permission = 'w' AND s_id = '0' AND s_name = '" . $array_folder[$y] . "' ", $EWT_DB_USER);
						} else if ($PP[1] == "dl") {
							$sql_sadmin = $db->query("SELECT * FROM permission WHERE p_type = '" . $type . "' AND pu_id = '" . $id . "' AND UID = '" . $_SESSION["EWT_SUID"] . "' AND s_type = 'dlFo' AND s_permission = 'w' AND s_id = '0' AND s_name = '" . $array_folder[$y] . "' ", $EWT_DB_USER);
						}
			...
			// webboard w
			if ($PP[1] == "webboard" and $PP[2] == "w") {
				$sql_f_all = $db->query("SELECT s_id FROM permission WHERE p_type = '" . $type . "' AND pu_id = '" . $id . "' AND UID = '" . $_SESSION["EWT_SUID"] . "' AND s_type = 'Wb' AND s_permission = 'w' AND s_id = '0'");

				if ($db->db_num_rows($sql_f_all) > 0) {
					echo "<ul><li>ทุกหมวด</li></ul>";
				} else {
					$sql_cmsw = $db->query("SELECT s_id FROM permission WHERE p_type = '" . $type . "' AND pu_id = '" . $id . "' AND UID = '" . $_SESSION["EWT_SUID"] . "' AND s_type = 'Wb' AND s_permission = 'w' ");
					if ($db->db_num_rows($sql_cmsw) > 0) {
						echo "<ul>";
						while ($F = $db->db_fetch_row($sql_cmsw)) {
							$db->query("USE " . $_SESSION["EWT_SDB"]);
							$sql_f = $db->query("SELECT c_name FROM w_cate WHERE c_id = '$F[0]' ");
							if ($db->db_num_rows($sql_f) > 0) {
								$Fo = $db->db_fetch_row($sql_f);
								echo "<li style=\"padding-left:50px;\"><i class=\"fas fa-folder\"></i> " . $Fo[0] . "</li>";
							}
			...
			//webboard a
			if ($PP[1] == "webboard" and $PP[2] == "a") {
				$sql_f_all = $db->query("SELECT s_id FROM permission WHERE p_type = '" . $type . "' AND pu_id = '" . $id . "' AND UID = '" . $_SESSION["EWT_SUID"] . "' AND s_type = 'Wb' AND s_permission = 'a' AND s_id = '0'");
				if ($db->db_num_rows($sql_f_all) > 0) {
					echo "<ul><li>ทุกหมวด</li></ul>";
				} else {
					$sql_cmsw = $db->query("SELECT s_id FROM permission WHERE p_type = '" . $type . "' AND pu_id = '" . $id . "' AND UID = '" . $_SESSION["EWT_SUID"] . "' AND s_type = 'Wb' AND s_permission = 'a' ");
					if ($db->db_num_rows($sql_cmsw) > 0) {
						echo "<ul>";
						while ($F = $db->db_fetch_row($sql_cmsw)) {
							$db->query("USE " . $_SESSION["EWT_SDB"]);
