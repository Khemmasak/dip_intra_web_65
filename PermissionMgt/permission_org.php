...
<div class="container-fluid">
	...
	##===========================================================================================##
	$_sql = $db->query("SELECT wgm.* FROM web_group_member wgm WHERE wgm.ug_id = '{$_SESSION['EWT_SUID']}' AND wgm.ugm_tid IN (SELECT org_id FROM org_name g WHERE $where_user) ORDER BY wgm.ugm_type ASC LIMIT {$start},{$perpage}");
	$statement = "SELECT count(ugm_id) AS b FROM web_group_member wgm WHERE wgm.ug_id = '{$_SESSION['EWT_SUID']}' AND wgm.ugm_tid IN (SELECT org_id FROM gen_user g WHERE $where_user)";
	//}
		...
		<div class="col-md-12 col-sm-12 col-xs-12 m-b-sm">
			...
			<div class="card">
				...
				<div class="card-header">
					<div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12 m-b-sm">
							<h4>รายการสิทธิ์กลุ่มหน่วยงาน</h4>
							<p></p>
						</div>
						<div class="col-md-12 col-sm-12 col-xs-12 m-b-sm">
							<div class="col-md-12 col-sm-12 col-xs-12">
								<ol class="breadcrumb">
									<li><a href="permission_main.php">รายการสิทธิ์กลุ่มหน่วยงาน</a></li>
									<li class=""></li>
								</ol>
							</div>

							<div class="col-md-12 col-sm-12 col-xs-12 float-right text-right hidden-xs">
								<button type="button" class="btn btn-info  btn-ml" onClick="boxPopup('<?php echo linkboxPopup(); ?>pop_set_permission_org.php');" title="เพิ่มกลุ่มหน่วยงาน" target="_self">
									<i class="fas fa-plus-circle"></i>&nbsp;เพิ่มสิทธิ์กลุ่มหน่วยงาน
								</button>
							</div>
			...
			buttons: {
				confirm: {
					...
					action: function() {
						$.ajax({
							type: 'POST',
							url: 'func_delete_permission_org.php',
							data: {
								'id': id,
								'proc': 'DelPermisOrg'
							},
