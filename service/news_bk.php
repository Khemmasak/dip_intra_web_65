...
if(empty($_GET['id'])){
	...
	//$sub = sub($_GET['id']);	 
	//$wh = "ditp_contents_cate.cate_id IN ('{$_GET['id']}'".$sub.")";
$X = implode(",",$cate_id);
if(count($cate_id) > 1){ 
$wh = "ditp_contents_cate.cate_id IN ('{$_GET['id']}',".$X.")";
}else{
...
$sql_c = "SELECT 
      ...
      WHERE ".$date_begin.$date_end." {$wh} {$whe} AND ditp_contents.contents_status > 0 
