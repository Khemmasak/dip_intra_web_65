...
$objPHPExcel->getActiveSheet()->getStyle('A1:F1')->applyFromArray($border_setting);
$objPHPExcel->getActiveSheet()->getStyle('A1:F1')->applyFromArray($style_color);
...
$objPHPExcel->getActiveSheet()->getStyle('A2:F2')->applyFromArray($border_setting);
$objPHPExcel->getActiveSheet()->getStyle('A2:F2')->applyFromArray($style_color);
$objPHPExcel->getActiveSheet()->getStyle('A2:F2')->applyFromArray($styleArray2);
...
$objPHPExcel->getActiveSheet()->setCellValue('F4', "Module :");
$objPHPExcel->getActiveSheet()->getStyle('A4:F4')->applyFromArray($style_headder);
$objPHPExcel->getActiveSheet()->getStyle('A4:F4')->applyFromArray($border_setting);
...
while($i < sizeof($a_detail) ){
	...
	$objPHPExcel->getActiveSheet()->setCellValue('F'.strval($i+5), $a_module[$i]);
	$objPHPExcel->getActiveSheet()->getStyle('A'.strval($i+5).':F'.strval($i+5))->applyFromArray($border_setting);
	$i++;
}
...
header('Content-Disposition: attachment;filename="Report Systemlog Reader Stat.xls"');
