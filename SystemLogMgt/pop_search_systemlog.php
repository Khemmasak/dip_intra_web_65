...
<form name="form_main" id="form_main" method="GET" action="<?=getLocation('systemlog_main')?>"  >
...
$_sql = $db->query("SELECT * 
						...
						ORDER BY web_group_member.ugm_type ASC 
						");	
...
<div class="form-group row " >
<label for="banner_gid" class="col-sm-12 control-label"><b><?=$txt_systemlog_search_module;?>  :</b></label>
<div class="col-md-12 col-sm-12 col-xs-12" >
<select name="module_code" id="module_code" class="form-control"  >
<option value=""selected="" disabled="disabled" ><?=$txt_systemlog_select_module;?></option> 
<?php
$_sql_cate = $db->query("SELECT *
					FROM web_module_ewt					
					WHERE web_module_ewt.m_status = 'Y'
					ORDER BY web_module_ewt.m_id ASC ");
while($a_data_cate = $db->db_fetch_array($_sql_cate)){
	//$sel = ($a_data_cate[0] == trim($vdo_cid)) ? "selected":"";
?>
<option value="<?=$a_data_cate['m_code'];?>" <?=$sel;?> >
<img src="<?=$IMG_PATH;?><?=$a_data_cate['m_image'];?>" class="img-responsive " style="display:inline;margin: 0 1px;width:16px;height:16px;" />
<?=$a_data_cate['m_name'];?>
</option>
<?php
	}		
