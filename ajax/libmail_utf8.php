 ...
 *		$mail->Body( "mเนl en franเนais avec des accents", "iso-8859-1" );

 */

function Body( $body, $charset="utf8" )
...
//	$this->xheaders['To'] = implode( ", ", $this->sendto );
	...
	if( $this->charset != '' ) {
		...
		$this->xheaders['Content-Type'] = "text/html; charset=utf8$this->charset";
...

{
	...
	//echo $this->headers.'<hr/>';

	//echo mb_encode_mimeheader($this->headers);

	$res = @mail( $this->strTo, $this->xheaders['Subject'], $this->fullBody, $this->headers );

	//$res = @mail( $this->strTo, $this->xheaders['Subject'], $this->fullBody, mb_encode_mimeheader($this->headers) );
...

{
	...
	$this->fullBody .= "Content-Type: text/html; charset=utf8$this->charset\nContent-Transfer-Encoding: $this->ctencoding\n\n" . $this->body ."\n";
...
$m->From('webmaster@prd.go.th');

$m->To('ieaui@hotmail.com');

//$m->To('support@prd.go.th');

$m->Subject('the subject of the mail --  เธซเธฑเธงเธเนเธญ เธ เธฒเธฉเธฒเนเธ—เธข');	



$message= "Hello world!\nthis is a test of the Mail class\nplease ignore\nThanks.\n\nเธ—เธ”เธชเธญเธเธ เธฒเธฉเธฒเนเธ—เธข utf8 เธเนเธฒ";
...
//$m->Attach( 'icon_loading.gif', 'image/gif' ) ;	// attach a file of type image/gif
