...
ฉ 2009, InnovaStudio (www.innovastudio.com). All rights reserved.
************************************************************/
...
function bookmark_selection() {
    ...
    if (oSel.type == "None" || oSel.type == "Text") {
      this.rangeBookmark = oRange.getBookmark();
...
function doOnPaste()
  ...
  if(this.pasteTextOnCtrlV) 
    {
      this.doPasteText(); // Paste on text
	 // this.outerHTML(); //Paste on html
      return false;
    }
  }
function doPaste()
  ...
  for(var i=0;i<oEl.childNodes.length;i++)
    ...
    if(oNode.nodeType==1)//tag
      ...
      var sTagName = oNode.nodeName;
            ...
            if (sCloseTag.indexOf("<?phpxml:namespace") > -1) sCloseTag=sCloseTag.substr(sCloseTag.indexOf(">")+1);
...
function ddAction(tb, id, edt, sfx) {
  ...
  switch(btn) {
    ...
    case "mnuTableEdit": modelessDialogShow(e.scriptPath+"table_edit.htm",358,395); break;
