...
<div class="container">
				<div class="row">
					<div class="col-lg-12">
						<h3 hidden>head</h3>
						<form name="frmMain" id="frmMain" method="post" action="formgenerator_pre.php" enctype="multipart/form-data">
							<fieldset>
...
<script type="text/javascript">
	...
	function submit_frm() {
		/*if(document.frmMain.ckemail.checked==true){
			if($("#email").val() == ""){
				alert('กรุณาระบุอีเมล์ตอบกลับ');
				$("#email").focus();
				return false;
			}else{
				var goodEmail1 = 
			  	 email.value.match(/\b(^(\S+@).+((\.com)|(\.net)|(\.edu)|(\.mil)|(\.or.th)|(\.gov)|(\.org)|(\.co.th)|(\.go.th)|(\.localnet)|(\..{2,2}))$)\b/gi);
				 if (!goodEmail1){
					alert('กรุณากรอกอีเมล์ให้ถูกรูปแบบ');
				 	$("#email").focus();
				 	return false;
