...
$_sql = "SELECT * FROM " . E_DB_NAME . ".article_group WHERE c_show_document = 'Y' ORDER BY c_id ASC ";
...
foreach ($a_data as $key => $value) {
    ...
    $article_list_center[$value["c_id"]] = article::getSubArticleListLimit($start, $per_page, $value["c_id"]);
}
...
<div id="main" style="<?php echo $org_page[0]["c_show_org_chk"] == "Y" ? "margin-left: 300px;" : null; ?>">
                            ...
                            <?php foreach ($article_list_main_top[$value["c_id"]] as $k => $val) { ?>
                                ...
                                <?php } ?>
                                            ...
                                            <div class="post-body px-3 pt-1 news-hilight border-ra-bottom-15px">
                                                <h4 class="post-title" title="<?php echo mb_strimwidth($val['n_topic'], 0, 500, '...') ?>">
                                ...
                                <?php } ?>
                                                ...
                                                <div class="post-body px-3 pt-1 news-hilight border-ra-bottom-15px">
                                                    ...
                                                    <h4 class="post-title">
                                                        ...
                                                        <a href="<?php echo ($val['news_use'] == 1 ? $val['link_html'] : "news_view.php?n_id=" . $val['n_id'] . " "); ?>" class="d-inline-block" title="<?php echo mb_strimwidth($val['n_topic'], 0, 500, '...');  ?>">
                                                            <p class=" mb-0 color-news-hilight"><?php echo mb_strimwidth($val['n_topic'], 0, 50, '...'); //substr($val["n_topic"], 0, -150); ?></p>
                                                        </a>
                                                    </h4>
                                                    <div class="latest-post-meta ">
                                                        <div class="row">
    ...
    <div class="container">
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <?php foreach ($a_data as $key => $value) { ?>
                <li class="nav-item" role="presentation">
                    <a onclick="tabActive('<?php echo $key; ?>','<?php echo $value['c_id']; ?>');" class="nav-link <?php echo ($key == $_SESSION["EWT_ARTICLE_TAP_ACTIVE"] ? "active" : "") ?>" id="home-tab<?php echo $key; ?>" data-toggle="tab" href="#home<?php echo $key; ?>" role="tab" aria-controls="home<?php echo $key; ?>" aria-selected="true">
        ...
        <div class="tab-content" id="myTabContent">
            <?php foreach ($a_data as $key => $value) { ?>
                <div class="tab-pane fade <?php echo ($key == $_SESSION["EWT_ARTICLE_TAP_ACTIVE"] ? "show active" : "") ?>" id="home<?php echo $key; ?>" role="tabpanel" aria-labelledby="home-tab<?php echo $key; ?>">
                    <div class="row m-4">

                        <!-- ช่องแรกเนื้อข่าวเล็ก -->
                        <?php foreach ($article_list_center[$value["c_id"]]["data"] as $k => $val) { ?>
                            <?php
                            $n_owner = user::chkUser(array("gen_user_id" => $val["n_owner"]))[0];
                            $full_name = $n_owner["name_thai"] . ' ' . $n_owner["surname_thai"];
                            $fname = $n_owner["name_thai"];
                            ...
                            <div class="col-lg-4 col-md-6 col-sm-12 col-12 mb-4">
                                ...
                                <div class="row">
                                    ...
                                    <?php }?>
                                        ...
                                        <a href="<?php echo ($val['news_use'] == 1 ? $val['link_html'] : "news_view.php?n_id=" . $val['n_id'] . " "); ?>" class="txt-header-news mt-1" title="<?php echo mb_strimwidth($val['n_topic'], 0, 500, '...');  ?>">
                                            <?php echo mb_strimwidth($val['n_topic'], 0, 50, '...'); //substr($val["n_topic"], 0, -150); ?>
                                        </a>
                                        <div class="row">
        ...
        <?php echo pagination_ewt('more_news.php', 'c_id=' . $c_id, $page, $per_page, $article_list_center[$c_id]["countAll"]); ?>
