...
<form id="form_main" name="form_main" method="POST" action="<?=getLocation('func_add_gallery')?>" enctype="multipart/form-data" >
<input type="hidden" name="proc" id="proc"  value="Add_Gallery">
...
<input class = "form-control" type = "text" placeholder = "ชื่อภาพ" id= "gal_title" name = "gal_title" required = "required">
</div>
</div>
...
<div class="col-md-12 col-sm-12 col-xs-12" >
<textarea class="form-control" placeholder="รายละเอียด"  rows="6" id="gal_detail" name="gal_detail"></textarea>
</div>
</div>
...
<?php if($_GET['c_id']==$group['category_id']){$cate = "selected";}else{$cate = "";}?>
<option value="<?=$group['category_id'];?>" <?php echo $cate;?>><?=$group['category_name'];?></option>
<?php } ?>
<?php } ?>
</select>
</div>
...
<button onclick="JQAdd_Gallery($('#form_main'));" type="button" class="btn btn-success  btn-ml " >
