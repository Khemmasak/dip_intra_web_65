...
<script language="JavaScript">
		  function flashupload_file(){
					...
					if(mybrowser.indexOf('MSIE')>0){
						...
						document.write("<form name=\"createform\" method=\"post\" action=\"download_new_multi.php\">");
			...
			function create_fo(c){
				if(c.value == ""){
					...
					if(mybrowser.indexOf('MSIE')>0){
						...
						document.write("<form name=\"createform\" method=\"post\" action=\"download_function.php\">");
						document.write("<input name=\"Flag\" type=\"hidden\" id=\"Flag\" value=\"CreateFolder\">");
						document.write("<input name=\"direct\" type=\"hidden\" id=\"direct\" value=\"download_index.php?myfolder=<?php echo $_REQUEST["myfolder"]; ?>\">");
				...
				function upload_file(){
					...
					if(mybrowser.indexOf('MSIE')>0){
						...
						document.write("<form name=\"createform\" method=\"post\" action=\"download_new.php\">");
						document.write("<input name=\"Flag\" type=\"hidden\" id=\"Flag\" value=\"UploadFile007\">");
						document.write("<input name=\"direct\" type=\"hidden\" id=\"direct\" value=\"<?php echo $send_folder; ?>\">");
						document.write("<br><br><br><br><br><div align=\"center\"><font size=\"5\" face=\"Verdana, Arial, Helvetica, sans-serif\"><strong>Please Wait...</strong></font></div>");
						document.write("</form>"); 
...
<tr>
    <td height="40" style="font-size:12px;" bgcolor="F3F3EE">&nbsp;&nbsp;&nbsp;
  ...
  <tr>
    ...
    <tr>
    <td style="font-size:12px;" height="<?php if($db->check_permission("dl","w","")){ echo "40"; }else{ echo "1"; } ?>" bgcolor="F3F3EE">
	<span <?php if(!$db->check_permission("dl","w","")){ echo "style=\"display:none\""; } ?>>&nbsp;&nbsp;&nbsp;
	<?php 
	if($db->check_permission("dlFo","w","".$f_array[0]."")){ 
	?>
	<a href="#new" onClick="create_folder();"><img src="../images/content_new_folder.gif" width="24" height="24" border="0" align="absmiddle"> 
      ...
      Delete</a>&nbsp;&nbsp;&nbsp;
	<!--<a href="#new" onClick="flashupload_file();"><img src="../images/folder_upload.gif" width="24" height="24" border="0" align="absmiddle"> 
     Flash Upload File(s)
	</a>-->
	<?php }?>
	</span>
  ...
  <tr> 
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="2" style="font-size:12px;">
        <tr>
          ...
          <td width="90%">
		  <table width="90%" border="0" cellpadding="2" cellspacing="1" bgcolor="66667E">
              <tr>
                <td height="20" style="font-size:12px;" bgcolor="#FFFFFF">
                  <?php echo $text_f; ?></td>
              </tr>
            </table></td>
        </tr>
		...
		$objFolder = opendir($Current_Dir);
			...
			$f = 0;
			  ...
			  $FT= filetype($Current_Dir."/".$file);
				if($num_p>0 || $_SESSION["EWT_SMTYPE"] == "Y" || $num_allfile>0){//echo $file."<br>";
				  if($FT == "dir"){
					array_push ($array_folder,$file);
				  }else{
				//  $file_size = @filesize($Current_Dir."/".$file);
					$array_file[$f][0] = $file;
					$array_file2[$f][0] = iconv('tis-620','UTF-8',$file);
...
<?php
  ...
  $numfile = count($array_file);
			...
			$preview_path2 = $folder."/".$array_file2[$y][0];
			}else{
			$preview_path = $array_file[$y][0];
			$preview_path2 = $array_file2[$y][0];
			}
			$preview_path_en = base64_encode($preview_path);
			
  ?>
                <li style="font-size:12px;" id="ah<?php echo $i; ?>"  onClick="hlah('<?php echo $i; ?>','<?php echo $array_file2[$y][0]; ?>','<?php echo $preview_path_en; ?>','Fi')" onDblClick="Editfile('<?php echo $preview_path2; ?>','<?php echo $ftype[1]; ?>')"><img src="../images/content_page.gif" width="24" height="24" border="0" align="absmiddle" > <?php echo $array_file2[$y][0]; ?></li>
				<?php
  $i++; } 
				echo "
...
?>
              <table id="tbemp" width="98%" border="0" align="center" cellpadding="3" cellspacing="0">
                <tr> 
                  <td height="50" style="font-size:12px;" align="center">This folder is empty.</td>
                </tr>
              </table>
              <?php
  }else{
?>
  ...
  ?>
  <hr><strong style="font-size:12px;" >&nbsp;Total <?php echo $numfolder; ?> 
