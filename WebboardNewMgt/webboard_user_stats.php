...
$_sql = $db->query("SELECT w_cate.*, sum(t_count) AS stat
					FROM w_cate
					LEFT JOIN w_question ON w_cate.c_id = w_question.c_id
					{$wh}
					GROUP BY w_cate.c_id
	...
	<div class="card-body">
		...
		<div class="panel-group" id="accordion" align="center">
			...
			<table border="1" width="90%">
				<tr>
					<th width="60%"><center>หมวดกระทู้</center></th>
					<th width="20%"><center>ผู้เข้าชมทั้งหมด</center></th>
				</tr>
				...
				<?php 
					while($a_data = $db->db_fetch_array($_sql)){
						if($a_data['stat']==0){
							...
							$a_data['stat']=$a_data['stat'];
						}
				?>
					<tr>
						<td><?php echo $a_data['c_name'] ;?></td>
						<td align="center"><?php echo $a_data['stat'] ;?></td>
					</tr>
				<?php
					}
				?>
		...
		<div class="panel-group" id="accordion" align="right">
			<a href="webboard_user_stats_cvs.php" title=""  target="_self">
				<button type="button" class="btn btn-default btn-sm" >
					<i class="far fa-arrow-alt-circle-right"></i>&nbsp;CVS
				</button>
			</a>
		</div>
	</div>
