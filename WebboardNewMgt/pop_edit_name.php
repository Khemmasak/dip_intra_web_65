...
$w_name_id = $_GET[w_id];
$select_name = $db->query("SELECT * FROM w_name WHERE w_name_id='$w_name_id'");

$select      = $db->db_fetch_array($select_name);

?>

<form id="form_main" name="form_main" method="POST" action="func_edit_name.php" enctype="multipart/form-data" >
	<input type="hidden" name="proc" id="proc"  value="Edit_Name">
	<input type="hidden" name="w_name_id" id="w_name_id"  value="<?php echo $select[w_name_id]; ?>">
	...
	<input class = "form-control" type = "text" placeholder = "ชื่อที่ห้ามใช้" id= "no_name" name = "no_name" required = "required" value="<?php echo $select[w_name]; ?>">
	</div>
	</div>
