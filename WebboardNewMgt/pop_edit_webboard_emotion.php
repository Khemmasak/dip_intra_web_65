...
$emo_id = (int)(!isset($_GET['emo_id']) ?  : $_GET['emo_id']);
if(empty($emo_id))
{
	exit();
}
$_sql = $db->query("SELECT * 
					FROM emotion 
					WHERE  emotion_id = '{$emo_id}'
					");					
$a_data = $db->db_fetch_array($_sql); 
$dir_base = "../ewt/".$_SESSION['EWT_SUSER']."/"; 
?>

<form id="form_main" name="form_main" method="POST" action="<?php echo getLocation('func_edit_webboard_emotion')?>" enctype="multipart/form-data" >
<input type="hidden" name="proc" id="proc"  value="Edit_Emo">
<input type="hidden" name="emo_id" id="emo_id"  value="<?php echo $emo_id;?>"> 
<input type="hidden" name="emotion_img_old" id="emotion_img_old"  value="<?php echo $a_data['emotion_img'];?>" >
<div class="container" >    
...
<h4 class="modal-title color-white"><i class="fas fa-plus-circle"></i> <?php echo 'แก้ไข emotion';?></h4> 
</div>
...
<div >
<label class="switch">
  <input type="checkbox" value="Y" name="emotion_status" id="emotion_status" <?php if($a_data['emotion_status'] == 'Y'){ echo 'checked="checked"'; } ?>>
...
<div class="col-md-12 col-sm-12 col-xs-12" >
<textarea class="form-control" placeholder="<?php echo 'โค้ด';?>"  rows="2" id="emotion_character" name="emotion_character"  required="required" ><?php echo $a_data['emotion_character'];?></textarea>
</div>
</div>
<div class="form-group row">
<div class="col-md-12 col-sm-12 col-xs-12"> 
<label for="emotion_name" class="col-sm-12 control-label"><b><?php echo 'รายละเอียด';?> <span class="text-danger"><code>*</code></span> :</b></label>
<textarea class="form-control" placeholder="<?php echo 'รายละเอียด';?>"  rows="5" id="emotion_name" name="emotion_name" required="required"><?php echo $a_data['emotion_name'];?></textarea>
</div>
</div>
...
<?php 
if(file_exists($dir_base.$a_data['emotion_img']) && $a_data['emotion_img'] != ''){
?>
<img src="<?php echo $dir_base.$a_data['emotion_img'];?>" alt="" class="img-rounded img-responsive file-upload-image-old" >
<?php } ?>
</div>
