...
<ol class="breadcrumb">
	...
	<li class=""><?=$txt_webboard_delete_notify_section;?></li>
</ol>
</div>
	...
	<div class="card ">
		<div class="card-header ewt-bg-color m-b-sm" >
			<div class="card-title text-left color-white">
				<p><h4><?=$txt_webboard_delete_notify_section;?></h4></p>
			</div>
		</div>
			...
			?>	
				...
				<div class="panel panel-default ">
					...
					<div class="panel-heading ewt-bg-success">
						...
						<b> Type: </b>
						<?php if($a_data['request_type']=="T"){ ?> กระทู้<?php } ?>
						<?php if($a_data['request_type']=="A"){ ?> ความคิดเห็น<?php } ?><br><br>

						<?php if($a_data['t_name']){ ?>
						<b> Topic: </b><?=$a_data['t_name'];?> (<a class="attachment_link" target="_blank" href="webboard_question.php?t_id=<?=$a_data['t_id'];?>">คลิกเพื่อตรวจสอบกระทู้</a>)<br>
						<?php } ?>

						<?php if($a_data['request_type']=="T"){ ?><!--<b> TID: </b> <?= $a_data['t_id']; ?><?php if(!$a_data['t_id']){ echo "-";} ; ?>--><?php } ?>
						<?php if($a_data['request_type']=="A"){ ?>
						<!-- <b> AID: </b> <?= $a_data['a_id']; ?><?php if(!$a_data['a_id']){ echo "-";} ; ?><br> -->
						<b> Detail: </b> <?= comment_detail($a_data['a_id']); ?><br>
						<?php } ?>
						...
						<b> Reason: </b><?=$a_data['request_reason'];?> <br><br><br>
						<b> Create Date/Time: </b><?=$a_data['request_createdate'];?> &nbsp;
						<b> IP Address: </b><?=$a_data['requestor_ip'];?>  <br>
					...
					<div class="panel-footer ewt-bg-white text-right" style="background-color: #FFC153;">
						...
						<div class="ewt-icon-wrap ewt-icon-effect-1 ewt-icon-effect-1b">
							...
							<button 
								type="button" class="delete_request btn btn-default  btn-circle  btn-sm " 
								data-toggle="tooltip" data-placement="top" 
								title="<?php echo $txt_webboard_delete_notify_text; ?>" value="<?=$a_data['request_id'];?>">
...
<?=pagination_ewt($statement,$perpage,$page,$url='webboard_delete_notify.php?');?>	
</div>
</div>
