...
function list_room($c_id,$depth){
  ...
  $order_subcate = 1;
  	...
  	while($subcate = $db->db_fetch_array($result_subcate)){
		...
		<input name="cate[]" value="<?php echo $subcate['c_id'];?>" type="checkbox"> <?php echo $subcate[c_name]; ?><br>
...
if($_POST["sadmin_name"]){
	...
	if($num_sadmin == 0){
		...
		$query_add_sadmin ="INSERT INTO w_admin(t_id,t_name, t_type, t_login) 
	              VALUES ('".$_POST['hdd_uid']."','".$_POST['sadmin_name']."','N','".$date."')";
		$db->query($query_add_sadmin);
		
		$db->write_log("add","webboard admin","เพิ่ม webboard admin ".$_POST['sadmin_name']);
	
		foreach ($_POST['cate'] as $value) {
		
			##ตรวจข้อมูลซ้ำ##
			
			$query_ch_permission = "SELECT t_id FROM w_permission WHERE t_id = '".$_POST['hdd_uid']."' AND c_id = '".$value."'";	
			$ch_permission_rows = $db->query($query_ch_permission);
			$num_permission = $db->db_num_rows($ch_permission_rows);
			
			if($num_permission==0){
			
				$query_permisssion = "INSERT INTO w_permission(t_id, c_id) 
									VALUES ('".$_POST['hdd_uid']."','".$value."')";
				$db->query($query_permisssion);
			}
		}
		...
		echo "<script>";
		echo "alert('ผู้ดูแลเว็บบอร์ดนี้มีอยู่ในระบบแล้ว');";
		echo "</script>";
	}
...
<ol class="breadcrumb">
	...
	<li><a href="webboard_add_subadmin.php?c_id=<?=$room_cid;?>">เพิ่มผู้ดูแลหมวดกระทู้</a></li>
</ol>
...
<div class="card ">
	<div class="card-header ewt-bg-color m-b-sm" >
		<div class="card-title text-left color-white">
			<p><h4>เพิ่มผู้ดูแลหมวดกระทู้</h4></p>
		</div>
	</div>
	<div class="card-body">
	...
	<form name="form1" method="post" action="#">
				...
				<table border="0" width="80%">
					<tr>
						...
						<td width="80%" style="padding: 10px">
							...
							<input type="text" name="sadmin_name" style="width: 75%" required>
							
							<a href="#" onClick="popo=window.open('site_s_member.php','popug','width=800,height=600,scrollbars=1,resizable=1');popo.focus();"><img src="../images/user_pos.gif" alt="เลือกจากบุคคลภายในองค์กร" title = "เลือกจากบุคคลภายในองค์กร" width="20" height="20" border="0" align="absmiddle"></a>&nbsp;
							
							<a href="#" onClick="popo=window.open('site_s_professor_c.php','popug','width=800,height=600,scrollbars=1,resizable=1');popo.focus();">
							<img src="../images/bar_user.gif" alt="เลือกจากรายการผู้เชี่ยวชาญของระบบกระทู้" title = "เลือกจากรายการผู้เชี่ยวชาญของระบบกระทู้" width="20" height="20" border="0" align="absmiddle">
							</a>
		...
		<div class="float-center text-center" style="top:18px;margin-bottom:10px;"> 
			<button id="submit_button" type="submit" class="btn btn-default btn-sm" >
				<i class="far fa-arrow-alt-circle-right"></i>&nbsp;เพิ่มผู้ดูแลหมวดกระทู้
			</button>
		</div>
	</form>
