...
if ($_POST["proc"] == "delete_room") {
	...
	if ($_POST['c_id']) {
		...
		$db->query($sql_del);
		$db->write_log("delete", "webboard", "ลบห้องสนทนา");
...
?>
	...
	if ($_POST['c_id']) {
		...
		$db->write_log("approve", "webboard", "อนุมัติห้องสนทนา");
	}
	?>
	<script>
		location.href = "webboard_main.php";
...
<?php
	...
	if ($_POST['c_id']) {
		...
		$db->query($sql_approve);
		$db->write_log("disapprove", "webboard", "ยกเลิกการอนุมัติห้องสนทนา");
	}
?>
	<script>
		location.href = "webboard_main.php";
					...
					<div class="row">
						...
						<div class="col-md-12 col-sm-12 col-xs-12 m-b-sm">
							<div class="col-md-12 col-sm-12 col-xs-12">
								<ol class="breadcrumb">
									<li><?php echo 'หมวดกระทู้'; ?></li>
								</ol>
							</div>

							<div class="col-md-12 col-sm-12 col-xs-12 float-right text-right hidden-xs">
								<button type="button" class="btn btn-info  btn-ml" onClick="boxPopup('<?php echo linkboxPopup(); ?>pop_add_webboard_room.php');">
									<i class="fas fa-plus-circle"></i>&nbsp;<?php echo $txt_webboard_new_room; ?>
								</button>
							</div>
							...
							<div class="col-md-6 col-sm-6 col-xs-12 float-right text-right visible-xs">
								<div class="btn-group ">
									...
									<ul class="dropdown-menu dropdown-menu-right">
										<li><a onClick="boxPopup('<?php echo linkboxPopup(); ?>pop_add_calendar_group.php?cal_cid=<?php echo $cal_cid; ?>');">
					...
					<div class="row ">
						<div class="col-md-12 col-sm-12 col-xs-12" id="table-view">
							<div class="panel-group" id="accordion">
								<?php
								if ($a_rows > 0) {
									if ($db->check_permission("Wb", "w", "0") || $db->check_permission("Wb", "a", "0")) {
										$pass = 'Y';
									}

									$i = 0;
									while ($a_data = $db->db_fetch_array($_sql)) {
										if (($db->check_permission("Wb", "w", $a_data['c_id']) || $db->check_permission("Wb", "a", $a_data['c_id']))  ||   $pass == 'Y') { ?>
											...
											table border="0" width="100%">
															<tr>
																...
																<td align="right">
																	<a href="webboard_room.php?c_id=<?php //echo $a_data['c_id'];
																									?>">
																		<button type="button" class="btn btn-default btn-sm" >
																			<i class="far fa-arrow-alt-circle-right"></i>&nbsp;<?php //echo $txt_webboard_read_room;
																																?>
																		</button>
																	</a>
																</td>
												...
												<div class="panel-heading ewt-bg-success">
													<h4 class="panel-title">
														<a class="accordion-toggle collapsed" data-toggle="collapse" data-parent="#accordion" href="#collapseOne<?php echo $i; ?>">
															...
															<div class="blockico color_ewt"><i class="fas fa-database" style="color:#FFBE7D;"></i></div>
															<?php echo $a_data['c_name']; ?>
														</a>
														<span class="pull-right">
														<a onClick="boxPopup('<?php echo linkboxPopup(); ?>pop_edit_webboard_main.php?c_id=<?php echo $a_data['c_id']; ?>')">
																	<button type="button" class="btn btn-warning  btn-circle btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo 'แก้ไขหมวดกระทู้'; ?>">
																		<i class="fas fa-edit" aria-hidden="true"></i>
																	</button>
																</a>
																<a href="webboard_main_room.php?c_id=<?php echo $a_data['c_id']; ?>">
																	<button type="button" class="btn btn-info btn-circle btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo $txt_webboard_read_room; ?>">
																		<i class="fa fa-th-list " aria-hidden="true"></i>
																	</button>
																</a>
																<a onClick="JQDel_Cate('<?php echo $a_data['c_id']; ?>');">
																	<button value="<?php echo $a_data['c_id']; ?>" type="button" class="delete_room btn btn-danger btn-circle btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo $txt_webboard_delete_room; ?>">
												...
												<div id="collapseOne<?php echo $i; ?>" class="panel-collapse collapse">
													<div class="panel-body">
														<div class="m-b"><b><i class="fas fa-angle-right" style="color:#FFBE7D;"></i>
																รายละเอียด&nbsp;:</b> <?php echo  $a_data['c_detail']; ?>
														</div>
														<?php
														$total_row = 0;
														...
														?>
														<div class="m-b"><b><i class="fas fa-angle-right" style="color:#FFBE7D;"></i>
																จำนวนกระทู้&nbsp;:</b><?php total_question_number($a_data["c_id"], $question_array); ?>
														</div>
														<div class="m-b"><b><i class="fas fa-angle-right" style="color:#FFBE7D;"></i>
																สถานะการใช้งาน&nbsp;:</b>
													...
													<?php total_question_number($a_data["c_id"], $question_array); ?> <br>
													<b>Status:</b> 
													<?php //if($a_data['c_use']!="Y"){ 
													?>
													<span style="color:red;">ไม่ใช้งาน</span>
													...
													<div class="panel-footer ewt-bg-white text-right">
														<div class="ewt-icon-wrap ewt-icon-effect-1 ewt-icon-effect-1b">
															<div class="text-right">
																<?php /*if($a_data['c_use']!="Y"){ ?>
																<button value="<?php echo $a_data['c_id'];?>" type="button" class="approve_room btn btn-default  btn-circle  btn-sm " data-toggle="tooltip" data-placement="top" title="<?php echo $txt_webboard_approve_room; ?>" >
																	<i class="far fa-check-circle"></i>
																</button>
															<?php } else { ?>
																<button value="<?php echo $a_data['c_id'];?>" type="button" class="disapprove_room btn btn-default  btn-circle  btn-sm " data-toggle="tooltip" data-placement="top" title="<?php echo $txt_webboard_disapprove_room; ?>" >
