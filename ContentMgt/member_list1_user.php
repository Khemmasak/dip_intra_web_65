...
if($_POST["Flag"] == "Add"){
   /*  $db->query("use  db_56_dmr_web");
	...
	$run .= " ORDER BY org_name.parent_org_id,gen_user.gen_user_id";
		$sql = $db->query($run);
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#CCCCCC"><form name="form1" method="post" target="_top" action="member_list1_user.php" >
  ...
  <tr bgcolor="#FFFFFF"> 
    <td><input type="checkbox" name="chk<?php echo $i; ?>" value="Y" <?php echo $chk_id;  ?>>
        ...
        <?php echo $U["name_thai"]; ?> <?php echo $U["surname_thai"]; ?> <input name="uid<?php echo $i; ?>" type="hidden" value="<?php echo $U["gen_user_id"]; ?>"> <input name="uname<?php echo $i; ?>" type="hidden" value="<?php echo $U["name_thai"]; ?> <?php echo $U["surname_thai"]; ?>">
      </td>
