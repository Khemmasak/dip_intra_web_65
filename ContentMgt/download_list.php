...
include("../lib/connect.php");
	...
	global $db;
			$sql_show = $db->query("SELECT * FROM docload_group ");
	...
	function showgroup($start,$i,$id,$idp,$dis){
			...
			$Blink = explode(',',$idp);
			$num = count($Blink);
			$sql_show = $db->query("SELECT * FROM docload_group WHERE dlg_parent = '".$start."' ");
			$total = $db->db_num_rows($sql_show);
			$dis += $total;
			if($total>0){
				while($R = $db->db_fetch_array($sql_show)){
					   $cat_id=$R[dlg_id];
					   $cat_name=$R[dlg_name];
						$sql_show_child= $db->query("SELECT * FROM docload_group WHERE dlg_parent = '".$cat_id."' ");
						$total_child = $db->db_num_rows($sql_show_child);
						echo GenPic($i).'<input type="checkbox" id='.$id.'  name="c_parent['.$id.']" value="'.$cat_id.'" ';
								
							if(in_array($cat_id, $Blink)){  echo ' checked '; 	}
...
$db->write_log("update","main","แก้ไข block doc download");
?>
<script language="JavaScript">
window.location.href = "download_list.php?B=<?php echo $_POST["B"]; ?>";
//self.close();
</script>
<?php
}else{
...
function disable_show(t){
	...
	if(t.checked==true){
		if(t.value == ''){
		    //alert(document.getElementById('numrowtotal').value);
...
<form name="form1" method="post" action="download_list.php">
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#666666">
  ...
  <tr bgcolor="#E6E6E6"> 
    <td><strong>Download group</strong></td>
  </tr>
  <tr bgcolor="#FFFFFF" > 
   <td><DIV style="HEIGHT: 370;OVERFLOW-Y: scroll;WIDTH: 100%;">
		  ...
		  <img src="../images/folder_closed.gif" width="16" height="16" border="0" align="absmiddle">หมวด Download ทั้งหมด<br>
