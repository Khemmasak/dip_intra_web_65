...
$graph_pic = "../ewt/".$_SESSION["EWT_SUSER"]."/graph/".$graph["graph_bgpic"];
}else{
$graph_pic = "";
}
...
if($type=="Column" OR $type=="Column3d"){
	...
	$graph->legend->SetFont(FF_ANGSANA,FS_NORMAL,13);
	$graph->xaxis->SetTickLabels($x_title);
	
	$graph->xaxis->title->SetFont(FF_ANGSANA,FS_NORMAL,13);
	$graph->xaxis->title->SetColor('black');
	$graph->xaxis->SetFont(FF_ANGSANA,FS_NORMAL,13);
	...
	$graph->title->SetFont(FF_ANGSANA,FS_BOLD,16);
	$graph->subtitle->SetFont(FF_ANGSANA,FS_NORMAL,14);
	...
	// Create the three var series we will combine
		...
		for($i=1;$i<($numrow_y+1);$i++){
			...
			${bplot.$i}->value->SetFont(FF_ANGSANA,FS_NORMAL,14);
	...
	// Create the three var series we will combine
		...
		for($i=1;$i<($numrow_y+1);$i++){
			...
			//${bplot.$i}->value->Show();
			//${bplot.$i}->value->SetFormat('0.0f');
			${bplot.$i}->value->SetFont(FF_ANGSANA,FS_NORMAL,13);
			${bplot.$i}->value->SetAngle(40);
			${bplot.$i}->value->SetColor("black","darkred");
			if($type=="Bar3d"){
			${bplot.$i}->SetShadow("black",5,5);
...
$graph->SetShadow();
	...
	//for($i=1;$i<($numrow_y+1);$i++){
			...
			$p1->value->SetFont(FF_ANGSANA,FS_NORMAL,13);
			$p1->SetLegends($y_titlepie);
			$p1->title->Set($x_title[0]);
			$p1->title->SetFont(FF_ANGSANA,FS_NORMAL,13);
