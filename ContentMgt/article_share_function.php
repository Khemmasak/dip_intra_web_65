...
for($i=0;$i<$_POST["allx"];$i++){
		...
		$N = $db->db_fetch_array($snews);
			...
			if($N[news_use] == "2" || $N[news_use] == "3"){
				...
				$url = "http://localhost/ewtadmin/ewt/".$_POST["USER"]."/ewt_news_body.php?nid=".$nid;
				$line = "";
				$fp = @fopen($url ,"r");
				if($fp){ 
		...
		//$text[$i] = "INSERT INTO article_list ( n_id , UID , user_share , n_date , n_timestamp , n_topic , n_des , source , sourceLink , keyword , picture , news_use , at_id , link_html , target , expire , logo , share_date ,n_owner) VALUES ( '$N[n_id]', '$_POST[UID]', '$_POST[USER]', '$N[n_date]', '$_N[n_timestamp]', '$N[n_topic]', '$N[n_des]', '$N[source]', '$N[sourceLink]', '$N[keyword]', '$N[picture]', '$N[news_use]', '$N[at_id]', '$linkhtml', '$N[target]', '$N[expire]', '$N[logo]', ".date('Y-m-d H:i:s')." , '$N[n_owner]')";
		//$db->query("UPDATE article_list SET n_share = 'Y' WHERE n_id = '".$nid."' ");
}
	...
	for($y=0;$y<$all;$y++){
		...
		if($chk != ""){
			$ex = explode("_",$chk);
				$UID_t = $ex[0];
				$CID = $ex[1];
				$sql = $db->query("SELECT EWT_User,db_db FROM user_info WHERE UID = '".$UID_t."' ");
