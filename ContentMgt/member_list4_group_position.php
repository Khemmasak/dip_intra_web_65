...
if($_POST["Flag"] == "Add"){
   /*  $db->query("use  db_56_dmr_web");
	...
	$run = "SELECT * FROM emp_type  "; 
		if($_POST["fname"] != ""){
			$run .= " WHERE emp_type.emp_type_name LIKE '%".$_POST["fname"]."%' ";
		}
	$run .= " ORDER BY emp_type.emp_type_id";
		$sql = $db->query($run);
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#CCCCCC"><form name="form1" method="post" target="_top" action="member_list1_user.php" >
  ...
  ?>
  <tr bgcolor="#FFFFFF"> 
    <td><input type="checkbox" name="chk<?php echo $i; ?>" value="Y" <?php echo $chk_id;?>>
        <img src="../images/user_pos.gif" width="20" height="20" border="0" align="absmiddle"> 
        <?php echo $U["emp_type_name"]; ?>  <input name="uid<?php echo $i; ?>" type="hidden" value="<?php echo $U["emp_type_id"]; ?>"> <input name="uname<?php echo $i; ?>" type="hidden" value="<?php echo $U["emp_type_name"]; ?> ">
      </td>
    <td>&nbsp;</td>
  </tr>
  <?php $i++; }}else{ ?>
