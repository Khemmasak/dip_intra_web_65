...
function send(){
var c = document.form1.use.length;
	for(i=0;i<c;i++){
		if(document.form1.use[i].checked == true){
		document.form1.txtuse.value = document.form1.txtuse.value + "@" + document.form1.use[i].value;
		}else{
		document.form1.txtnouse.value = document.form1.txtnouse.value + "@" + document.form1.use[i].value;
		}
	}
form1.submit();
}
...
<form name="form1" method="post" action="article_share_function1.php"><tr>
<?php
...
<input type="hidden" name="UID" value="<?php echo $M[0]; ?>">
			...
			<input type="hidden" name="txtnouse" value="">
			<input type="hidden" name="txtuse" value="">
