...
if($_POST[article_s_name] != ''){
		...
		$acticle_name = str_replace('*','\"*\"',$acticle_name);
$wh1 .= " AND (article_list.n_topic REGEXP '".$acticle_name."' or article_list.n_des REGEXP '".$acticle_name."') AND";
}
...
$ed = explode("/",$ed);
$ed = ($ed[2] )."-".$ed[1]."-".$ed[0];
$wh1 .= " AND   (article_list.n_date  between '".$st."' AND '".$ed."'  ) AND";
}
...
$ed = explode("/",$ed);
$ed = ($ed[2] )."-".$ed[1]."-".$ed[0];
$wh1 .= " AND   (article_list.n_date  between '".$ed."' AND '".$ed."'  ) AND";
}
if($_POST[start_date] != '' && $_POST[end_date]==''){
$st = stripslashes(htmlspecialchars(trim($_POST[start_date]),ENT_QUOTES));
$st = explode("/",$st );
$st = ($st[2] )."-".$st[1]."-".$st[0];
$wh1 .= " AND   (article_list.n_date  between '".$st."' AND '".$st."'  ) AND";
}
if($_POST[article_gruop] != ''){
$article_gruop = $_POST[article_gruop];
$wh1 .= "  AND  article_list.c_id = '".$article_gruop."' AND";
}
$wh="news_use='4' " ;
$wh .= substr($wh1,0,-3);
if($wh != ''){
...
$sql_article = $db->query("SELECT news_view.news_id, count(news_view.id_view) AS count_view, article_list.n_topic,c_id
								          ...
								          order by count_view desc $list_limit ");

/*
echo   "SELECT news_view.news_id, count(news_view.id_view) AS count_view, article_list.n_topic,c_id
													FROM article_list  INNER JOIN news_view ON (article_list.n_id = news_view.news_id) $wh
													GROUP BY news_view.news_id,  article_list.n_topic
								          order by count_view desc $list_limit ";
										  */
...
<table width="80%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr> 
    <td><img src="../theme/main_theme/article_logo.gif" width="32" height="32" align="absmiddle"> 
      <span class="ewtfunction">รายงานผู้ Download ข่าว/บทความ  <?php echo $lablereting;?></span> </td>
  </tr>
</table>
...
<table width="94%" border="0" align="center" cellpadding="3" cellspacing="0" class="ewtfunctionmenu">
  <tr> 
    <form name="form2" method="post" action="article_download_stat.php"  >
