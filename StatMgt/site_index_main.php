...
<table width="94%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#CECECE" class="ewttableuse">
  ...
  $sql = "select * from list_name where list_type ='stat_visitor' and list_site = '".$EWT_FOLDER_USER."' order by list_id ASC";
  $query = $db->query($sql);
  while($R = $db->db_fetch_array($query)){
  ?>
  <tr bgcolor="#FFFFFF"> 
    <td align="center"><nobr><a href="site_index_view4.php?db=<?php echo base64_encode ( $R["list_id"]."EX".$R["list_dbname"]."EX"); ?>"><img src="../theme/main_theme/g_view.gif" width="16" height="16" border="0" align="absmiddle" alt="ดูข้อมูล"></a> </nobr></td>
