...
$_sql = $db->query("SELECT * 
					FROM cal_google  
					ORDER BY cal_google_id DESC LIMIT {$start} , {$perpage} ");

$statement = "SELECT count(cal_google_id) AS b
			  FROM cal_google 
			  ";
...
<ol class="breadcrumb">
<li><a href="calendar_google.php"><?=$txt_calendar_google;?></a></li>
<li class=""></li>       
</ol>
</div>
...
<button type="button" class="btn btn-info  btn-ml" onClick="boxPopup('<?=linkboxPopup();?>pop_add_calendar_google.php');" title="<?=$txt_calendar_add;?>" >
<i class="fas fa-plus-circle"></i>&nbsp;<?=$txt_calendar_add_google;?>
</button>

<!--<button type="button" class="btn btn-info  btn-ml " onclick="boxPopup('<?=linkboxPopup();?>onclick="boxPopup('<?=linkboxPopup();?>pop_search_calendar.php');".php');" title="<?=$txt_calendar_search_google;?>" >
<i class="fas fa-search"></i>&nbsp;<?=$txt_calendar_search_google;?>
</button>-->
...
<div class="btn-group ">
        ...
        <ul class="dropdown-menu dropdown-menu-right pointer">
			<li><a onClick="boxPopup('<?=linkboxPopup();?>pop_add_calendar_google.php');" ><i class="fas fa-plus-circle"></i>&nbsp;<?=$txt_calendar_add_google;?></a></li>
...
<div class="panel panel-default ">
            <div class="panel-heading ewt-bg-success">
                <h4 class="panel-title">
					...
					<?=$a_data['cal_google_name'];?>
					</a>
					<?php if($a_data['cal_google_status'] == 'Y'){ 
					echo '<i class="far fa-check-circle color-ewt"></i>'; 
					}
					?>					
                </h4>
            </div>
...
<a onClick="JQDelete_CalendarGoogle(<?=$a_data['cal_google_id']?>);"  > 
<button type="button" class="btn btn-default  btn-circle  btn-sm " data-toggle="tooltip" data-placement="top" title="<?=$txt_calendar_delete_google;?>" >
<i class="far fa-trash-alt" aria-hidden="true"></i>
</button>

<a onClick="boxPopup('<?=linkboxPopup();?>pop_edit_calendar_google.php?cal_google_id=<?=$a_data['cal_google_id'];?>');" >					
<button type="button" class="btn btn-default btn-circle btn-sm " data-toggle="tooltip" data-placement="top" title="<?=$txt_calendar_edit_google;?>" >
...
<script>
function JQDelete_CalendarGoogle(id){
					$.confirm({
						title: 'ลบข้อมูล',
						...
						buttons: {
								confirm: {
									...
									action: function () {
										$.ajax({
											type: 'POST',
											url: 'func_delete_calendar_google.php',
											data:{'id': id,'proc':'DelCalGoogle'},
