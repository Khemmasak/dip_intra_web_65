...
<div class="container-fluid">
	...
	function countparent($c)
	{
		...
		while ($U = $db->db_fetch_array($sql)) {
			...
			AND ((s_type = '" . $ptype . "' AND s_permission = '" . $ppms1 . "'  AND s_id = '" . $U["c_parent"] . "' ) OR 
	   		(s_type = '" . $ptype . "' AND s_permission = '" . $ppms2 . "'  AND s_id = '" . $U["c_parent"] . "' )) ", $EWT_DB_USER);
	...
	OR (p_type = 'D' AND pu_id = '".$_SESSION["EWT_SMDIV"]."'))  
	AND UID = '" . $_SESSION["EWT_SUID"] . "' 
	AND ((s_type = '" . $ptype . "' AND s_permission = '" . $ppms1 . "'  AND s_id != '0' )  OR
	(s_type = '" . $ptype . "' AND s_permission = '" . $ppms2 . "'  AND s_id != '0' ))", $EWT_DB_USER);
	...
	$statement = "SELECT count(c_id) AS b FROM article_group {$wh} {$sql_search}";
					...
					<div class="row">
						...
						<div class="col-md-12 col-sm-12 col-xs-12 m-b-sm">
							...
							<div class="col-md-12 col-sm-12 col-xs-12 float-right text-right hidden-xs">
								<?php if($db->check_permission("suser", "", "0")) { ?>
														...
														<tr class="productCategoryLevel1-td move" id="<?php echo $G['c_id']; ?>">
															...
															<td class="text-left" colspan="2">
																...
																<a href="article_list.php?cid=<?php echo $G['c_id']; ?>">
																	...
																	$allnews_data  = $db->query("SELECT COUNT(n_id) AS total_allnews FROM article_list WHERE c_id = '{$G['c_id']}' AND n_approve = 'Y' AND n_approve != 'D'");
																	$allnews_info  = $db->db_fetch_array($allnews_data);
																	?>
																	<!-- [<b><?php echo number_format($totalsubgroup); ?></b> กลุ่ม <b><?php echo number_format($allnews_info["total_allnews"]); ?></b> บทความ] -->
																</a>
															</td>
															...
															<td class="text-center" <?php echo $disabled1; ?>>
																...
																?>
																	<div class="checkbox">
																		<label>
																			<input name="chk<?php echo $i; ?>" type="checkbox" id="chk<?php echo $i; ?>" value="<?php echo $G['c_id']; ?>" <?php echo (number_format($totalsubgroup) > 0 || number_format($allnews_info["total_allnews"]) > 0) ? "disabled" : null; ?> />
																			<span class="cr"><i class="cr-icon fas fa-check color-ewt"></i></span>
																		</label>
																	</div>
																<?php } ?>
