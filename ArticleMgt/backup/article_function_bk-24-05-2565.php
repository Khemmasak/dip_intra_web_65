...
?><script language="javascript">
		...
		<?php
				...
				{    
					...
					{		
						...
						c_show_org_menu = '" . $_POST["menu_org"] . "' ,
						c_show_org_banner = '" . $_POST["banner_org"] . "' ,
						c_show_org_image = '" . $a_attach . "',
						c_show_org1 = '" . $_POST["org_group1"] . "' ,
						c_show_org2 = '" . $_POST["org_group2"] . "' ,
						c_show_org3 = '" . $_POST["org_group3"] . "' ,
						c_type = '" . $_POST["gtype"] . "' ,
						d_id = '" . $_POST["c_order"] . "' ,
	...
	?>
		...
		?>
			...
			<?php
				...
				if ($_SESSION["EWT_SMID"] != "") {
					...
					// $org = $db->query("SELECT org_name.name_org, org_name.org_id FROM `gen_user`  INNER JOIN `org_name` ON (`gen_user`.`org_id` = `org_name`.`org_id`)  WHERE gen_user_id = '" . $_SESSION["EWT_SMID"] . "'");
					// $O = $db->db_fetch_row($org);
					// $org_own = $O[0];
					// $org_id = $O[1];
					$sql_user = $db->query("SELECT gen_user FROM gen_user WHERE gen_user_id = '" . $_SESSION["EWT_SMID"] . "'");
					$result_user = $db->db_fetch_array($sql_user);

					$sql_sso = "SELECT DEP_ID FROM USR_MAIN WHERE USR_USERNAME = '".$result_user["gen_user"]."' ";
					$result_sso = $sso->getFetch($sql_sso);

					$sql_org = "SELECT DEP_ID, DEP_NAME FROM USR_DEPARTMENT WHERE DEP_ID = '".$result_sso["DEP_ID"]."' ";
					$result_org = $sso->getFetch($sql_org);

					$org_own = ($_SESSION["EWT_SMID"] == 0) ? "ศูนย์เทคโนโลยีสารสนเทศการประชาสัมพันธ์" : $result_org["DEP_NAME"];
					$org_id = ($_SESSION["EWT_SMID"] == 0) ? 21 : $result_sso["DEP_ID"];
				...
				if ($appove_a == "Y" && $_POST['approve_user'] == '') {
					...
					## >> Get h_id
					$hid_data = $db->query("SELECT MAX(h_id) AS h_id FROM n_history");
		...
		<table bgcolor = "#FFFFFF" border = "0" width = "100%">
			...
			?>
			<script language="JavaScript">
				alert('Image file size of \"<?php echo $_FILES["file"]["name"] . "\" size[" . number_format($_FILES['file']['size'] / 1024, 2) . " Kb]"; ?> is over maximum.\nPlease change or resize your file.')
			</script>
		<?php
					}
				}
...
*/
		...
		?>
			<script language="javascript">
				window.open("../ewt/prd_intra_web/article_view.php?n_id=<?= $_POST['nid']; ?>", "artpv", "width=1000,height=550,resizable=1,scrollbars=1");
				// window.open("../ewt/<?php echo $_SESSION["EWT_SUSER"]; ?>/article-views-backend.php?nid=<?= $_POST['nid']; ?>&lang=TH", "artpv", "width=1000,height=550,resizable=1,scrollbars=1");
				//self.location.href = "article_detail.php?nid=<?php echo $_POST["nid"]; ?>";
			</script>
		<?php
						break;
	...
	if($_POST["n_action"] == "preview"){
			...
			?>
			<script language="javascript">
				window.open("../ewt/<?php echo $_SESSION["EWT_SUSER"]; ?>/news_view.php?n_id=<?php echo $_POST["nid"]; ?>","artpv","width=800,height=550,resizable=1,scrollbars=1");
		//		self.location.href = "article_detail.php?nid=<?php echo $_POST["nid"]; ?>";
			</script>
		<?php
	}	*/
			...
			if ($_POST["Flag"] == "SetDocument") {
				...
				for ($i = 0; $i < $_POST["alli"]; $i++) {
					...
					if ($chk != "") {
						...
						$db->query("Update article_group SET c_show_document=NULL WHERE c_id = '$nid'");
					}
				}
			...
			function Gen_RSS($cid)
			{
				...
				if ($rss2["c_rss"] == 'Y') {
					...
					while ($rss = $db->db_fetch_array($query_rss)) {
						//$Urls="http://61.91.248.62/ewtadmin/ewt/".$_SESSION["EWT_SUSER"]."/news_view.php?n_id=$rs[news_id]";
						$N = array();
