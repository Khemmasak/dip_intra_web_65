...
<div class="container-fluid">
	...
	$check_org = $article->getAricleCheckOrg($_GET['cid']);
	...
	$org_page = $org->getArticleOrg($_GET['cid']);
	$chk_display = $org_page["data"]["c_show_org_chk"];
	$chk_menu = $org_page["data"]["c_show_org_menu"];
	$chk_banner = $org_page["data"]["c_show_org_banner"];
	$chk_image = $org_page["data"]["c_show_org_image"];
	$chk_org1 = $org_page["data"]["c_show_org1"];
	$chk_org2 = $org_page["data"]["c_show_org2"];
	$chk_org3 = $org_page["data"]["c_show_org3"];
	//var_dump($org_page["data"]["c_show_org_menu"]); exit();
									...
									<form action="article_function.php" method="post" name="form1" id="form1" enctype="multipart/form-data">
										<div class="form-group row">
											<div class="col-md-8 col-sm-8 col-xs-8">
												<label for="c_name"><b><?= $txt_article_group_name; ?> <span class="text-danger"><code>*</code></span> : </b></label>
												<!--<input class="form-control" name="c_name" type="text" id="c_name"  value="<? //=$G['c_name']; 
																																?>" />-->
												<textarea class="form-control" placeholder="<?= $txt_article_group_name; ?>" rows="3" id="c_name" name="c_name" required="required"><?= $G['c_name']; ?></textarea>

											</div>
											<div class="col-md-4 col-sm-4 col-xs-12">
												<label for="c_name"><?= "อยู่ภายใต้หมวด"; ?> : </label><br>
												<input type="hidden" name="c_parent" value="<?= $G['c_parent']; ?>">
												<span id="gname">
													<?php echo $gname; ?>
												</span>
												<a href="#browse" title="เลือกกลุ่ม" onClick="win2 = window.open('article_select_edit.php?cid=<?= $_GET['cid']; ?>','WebsiteLink','top=100,left=200,width=900,height=500,resizable=1,status=0,scrollbars=1');win2.focus();">
													<button type="button" class="btn btn-info  btn-sm ">
														<span class="glyphicon glyphicon-folder-open"></span>&nbsp;เลือกกลุ่ม
													</button>
												</a>
											</div>
										</div>
										<?php //include "setting_gedit.php";?>
										<div class="form-group row">
											<div class="col-md-12 col-sm-12 col-xs-12" style="text-align:center;">
												<input type="submit" name="Submit2" value="&nbsp;&nbsp;บันทึก&nbsp;&nbsp" class="btn btn-success">
												<input name="cid" type="hidden" id="cid" value="<?= $_GET['cid']; ?>">
												<input name="Flag" type="hidden" id="Flag" value="EditGroup">
												<input name="gtype" type="hidden" id="gtype" value="<?= $G['c_type']; ?>">
											</div>
										</div>
									</form>
