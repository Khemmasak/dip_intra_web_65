...
<h4 class="modal-title">บริหารข่าว/บทความภาษาอื่นๆ</h4>
</div>
...
for($i=1;$i<=$a_rows;$i++){
	...
	$s_status = set_lang_use($_GET['id'],$a_data['lang_config_id'],'article_list');
	if($s_status == true){
		$disabled = 'disabled';
		}else{
			$disabled = '';
...
<button  type="button" class="btn btn-default  btn-ml" <?=$disabled?> data-toggle="tooltip" data-placement="top" title="<?=$a_data['lang_config_name'];?>" onClick="txt_data2('<?=$_GET['id'];?>','<?=$a_data['lang_config_id'];?>','<?=$a_data['lang_config_suffix'];?>');" >
