...
<tr> 
      ...
      <td width="43%" valign="top" bgcolor="#FFFFFF"><input name="subject" type="text" id="subject" size="40"></td>
    <?php
...
<tr>
  ...
  <td valign="top" bgcolor="#FFFFFF"><?php echo $lang_sendmail_name; ?>  <input name="from_name" type="text" id="from_name" value="<?php echo $mynameconfig; ?>" size="40"></td>
  </tr>
<tr>
  <td valign="top" bgcolor="#FFFFFF"><?php echo $lang_sendmail_email; ?>   <input name="from_email" type="text" id="from_email" value="<?php echo $myemailconfig; ?>" size="40"></td>
</tr>
<?php
...
$Ex = $db->query($Sel);
$RowNEWSGroup = mysql_num_rows($Ex);
?>
<tr>
...
document.all.shwc.style.display = 'none';
  ...
  <td colspan="2" bgcolor="#FFFFFF"><input name="cusmail" type="text" id="cusmail" size="110"></td>
  </tr>
<tr bgcolor="ECEBF0">
  <td valign="top" bgcolor="#FFFFFF"><?php echo $lang_sendmail_body; ?></td>
  <td colspan="2" valign="top" bgcolor="#FFFFFF"><select name="stype" id="stype" style="WIDTH: 250" onChange="SelT(this.value);">  
...
?>
      <tr>
        <td><select name="seltemp" style="WIDTH: 250" onChange="enb(this.value,form1.seltemp.options[form1.seltemp.selectedIndex].text);">  
		<option value=""><?php echo $lang_sendmail_select_temp; ?></option>
		  <?php while($RRR = mysql_fetch_array($Ex2)){ ?>
  <option value="<?php echo $RRR[t_id]; ?>"><?php echo $RRR[t_name]; ?></option>
  <?php } ?>
  </select>
          <input name="zzz" type="button" disabled id="zzz" value="<?php echo $lang_sendmail_edit_tempp; ?>" onClick="PopEdit();">
		  <?php
...
document.all.prev.src = "<?php echo $Path_true; ?>/temp_preview.php?tid="+ d;
}
}
function PopEdit(){
win2 = window.open('../ewt/<?php echo $EWT_FOLDER_USER;?>/temp_add.php?tid='+form1.seltemp.options[form1.seltemp.selectedIndex].value+'&Flag=Edit&module=1','TempEdit','height=550,width=600,resizable=1,scrollbars=1');
}
      </script>
	  <br>
