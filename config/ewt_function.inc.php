...
$txt .=$banner_ahref;
$txt .="<img src=\"".$a_banner['banner_pic']."\" class=\"img-responsive\" alt=\"".$a_banner['banner_name']."\" _title=\"".$a_banner['banner_name']."\" />";
$txt .=$banner_a;
$txt .="</div>".PHP_EOL;
...
$s_banner = $db->query("SELECT * FROM banner  
						...
						WHERE  banner.banner_gid = '{$bid}' {$wh} ORDER BY banner.banner_position ASC");
while($a_banner = $db->db_fetch_array($s_banner)){

if(!empty($a_banner['banner_w']) OR !empty($a_banner['banner_h'])){
	if(empty($a_banner['banner_w'])){
		$a_banner['banner_w'] = '100%';
	}else{
		$a_banner['banner_w'] = $a_banner['banner_w'].'px';
	}
	if(empty($a_banner['banner_h'])){
		$a_banner['banner_h'] = 'auto';
	}else{
		$a_banner['banner_h'] = $a_banner['banner_h'].'px';
	}	
$banner_w_h =  'style="width:'.$a_banner['banner_w'].';height:'.$a_banner['banner_h'].';"';
}
...
$txt .="<div class=\"item\">".PHP_EOL;
$txt .=$banner_ahref;
$txt .="<img class=\"d-block \" ".$banner_w_h." src=\"".$a_banner['banner_pic']."\" alt=\"".$a_banner['banner_name']."\" _title=\"".$a_banner['banner_name']."\" />	";
$txt .=$banner_a;
$txt .="</div>".PHP_EOL;
...
$txt .="<img class=\"img-fluid\" src=\"".$a_banner['banner_pic']."\" alt=\"".$a_banner['banner_name']."\" _title=\"".$a_banner['banner_name']."\" />	";
$txt .="</div>".PHP_EOL;
$txt .=$banner_a;
}
$txt .="</div>".PHP_EOL;
...
function ewt_lang_detail($c_id,$lang_name,$lang_field,$module){
global $db;
$tb = "lang_".$module;
...
$wh = "AND cal_event.event_date_start >= '".date('Y-m-d')."' ";
if(SYS_LANG != 'TH'){
$s_cal = $db->query("SELECT cal_event.*,
							...
							FROM lang_cal_event
							INNER JOIN cal_event      ON lang_cal_event.c_id      = cal_event.event_id
							INNER JOIN cal_show_event ON lang_cal_event.c_id      = cal_show_event.event_id
							INNER JOIN lang_config    ON lang_cal_event.lang_name = lang_config.lang_config_id
							INNER JOIN cal_category   ON cal_event.cat_id = cal_category.cat_id
							WHERE
							lang_config.lang_config_suffix = '".SYS_LANG."'
							AND lang_cal_event.lang_field = 'event_title'
							ORDER BY cal_show_event.event_date_start ASC LIMIT 0,3
							");
}else{
$s_cal = $db->query("SELECT cal_event.*,
							cal_show_event.event_date_start,
...
$txt .='<div class="col-11 col-sm-11 col-lg-11 pl-0">'.PHP_EOL;
	$txt .='<span class="badge badge-saminar badge-secondary mb-2">'.chg_date_cal($a_data['event_date_start'],$a_data['event_date_end']).'</span>'.PHP_EOL;
...
if(SYS_LANG != 'TH'){
$a_data['event_title'] = ewt_lang_detail($a_data['event_id'],SYS_LANG,'event_title',$modulename);
$a_data['event_location'] = ewt_lang_detail($a_data['event_id'],SYS_LANG,'event_location',$modulename);  
}
$txt .='<div class="col-lg-12 col-md-12 col-sm-12 col-12">'.PHP_EOL;
$txt .='<div class="card card-saminar shadow-image mb-4" style="border-radius: 8px;">'.PHP_EOL;
$txt .='<div class="card-body pt-3 pb-3">'.PHP_EOL;
$txt .='<div class="row" style="margin-left: -25px;">'.PHP_EOL;
$txt .='<div class="col-1 col-sm-1 col-lg-1 pl-0 pr-0">'.PHP_EOL;
...
$txt .='<div class="title-saminar">'.PHP_EOL;
...
$txt .='</button></div>'.PHP_EOL;
		...
		$txt .="<button type=\"button\" class=\"btn btn-saminar btn-danger\"  >";
		$txt .=$text_calendar_disable_regis.PHP_EOL;
		$txt .='</button>';
	}
}
if($a_data['event_registor'] == 'M'){
			if(!empty($a_data['event_link_registor'])){
$txt .="<div class=\"right-box\"><button type=\"button\" class=\"btn btn-saminar btn-success pad-btn-calendar\"  data-toggle=\"modal\" onClick=\"window.open('".$a_data['event_link_registor']."');\" >".PHP_EOL;
$txt .=$text_calendar_regis.PHP_EOL;
$txt .='</button></div>'.PHP_EOL;
		}
}
...
$txt .="<h4>".PHP_EOL;
$txt .=showWord($a_article['n_topic'],'165').PHP_EOL;
...
if($lastpage >= $page){
    ...
    if($lastpage > 1){
        ...
        if ($page > 1){
			$pagination.= '<li class="page-item" ><a class="page-link " href="'.$url.'page=1">'.$firstlabel.'</a></li>'; 
            ...
            if($page < 1 + ($adjacents * 2)) {
                ...
                for ($counter = $lastpage - (2 + ($adjacents * 2)); $counter <= $lastpage; $counter++) {
                    if ($counter == $page)
                        ...
                        $pagination.= '<li class="page-item " ><a class="page-link " href="'.$url.'page='.$counter.'#table-view">'.$counter.'</a></li>';
                }
            }
        }
		...
		if($lastpage == $page){
			$pagination.= '<li class="page-item disabled" ><a class="page-link ">'.$nextlabel.'</a></li>';
		}else{
			$pagination.= '<li class="page-item " ><a class="page-link " href="'.$url.'page='.$next.'">'.$nextlabel.'</a></li>';
		}
