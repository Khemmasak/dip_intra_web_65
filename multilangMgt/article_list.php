...
if($_POST[flag]=='set_lang'){
	...
	if($_POST[news_use] == '1'){
		if($_POST["browsefile"]=='1'){
			$_POST[lang_detail][9] = $_POST["link"];
			}else{
			...
			@mkdir ($Current_Dir2, 0777);
				...
				$_POST[lang_detail][9] = $Current_Dir3."/".$picname;
				}
			}
	}
	for($i=0;$i<$_POST[num];$i++){
...
$type = 'list';
$sql = $db->db_fetch_array($db->query("select c_id,n_date,link_html,n_topic,n_des,source,news_use,sourceLink,keyword,n_date_start,n_date_end,at_id from article_list where n_id = '".$_GET[id]."'"));
if($sql[at_id]==10){
?>
      <script language="JavaScript">
		   location.href='../ewt/<?php echo $EWT_FOLDER_USER;?>/article_list.php?langid=<?php echo $_GET[langid];?>&lang=<?php echo $_GET[lang];?>&id=<?php echo $_GET[id];?>';
     </script>
   <?php
   exit;
}
...
function chk(){
	if(document.getElementById("lang_detail"+"["+"0"+"]").value == ""){
		alert("Please insert languang!!");
		document.getElementById("lang_detail"+"["+"0"+"]").focus();
		return false;
	}
}
</script>
...
<form name="form1" method="post" action="article_list.php"  enctype="multipart/form-data" onSubmit="return chk();">
  ...
  <tr>
    <td width="129" height="11" valign="top" bgcolor="#FFFFFF">Topic   : <strong style="color:#FF0000">*</strong></td>
    <td width="336" height="0" valign="top" bgcolor="#FFFFFF"><input name="lang_detail[0]" type="text" size="50" value="<?php echo select_lang_detail($_GET[id],$_GET[langid],'n_topic','article_list');?>"><input type="hidden" name="lang_field[0]" value="n_topic"></td>
    <td width="1" valign="top" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="183" height="0" valign="top" bgcolor="#FFFFFF"><?php echo nl2br ($sql[n_topic]);?></td>
  </tr>
  <tr>
    <td height="12" valign="top" bgcolor="#FFFFFF">Description : <strong style="color:#FF0000">*</strong></td>
    <td width="336" height="0" valign="top" bgcolor="#FFFFFF"><input name="lang_detail[1]" type="text" id="lang_detail[1]" value="<?php echo select_lang_detail($_GET[id],$_GET[langid],'n_des','article_list');?>" size="50">
      <input type="hidden" name="lang_field[1]" value="n_des"></td>
    <td width="1" height="0" valign="top" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="183" height="0" valign="top" bgcolor="#FFFFFF"><?php echo nl2br ($sql[n_des]);?></td>
  </tr>
 <tr  style="display:none" >
    <td height="23" valign="top" bgcolor="#FFFFFF">Source   : <strong style="color:#FF0000">*</strong></td>
    <td width="336" height="0" valign="top" bgcolor="#FFFFFF"><input name="lang_detail[2]" type="text" size="50" value="<?php echo select_lang_detail($_GET[id],$_GET[langid],'source','article_list');?>">
      <input type="hidden" name="lang_field[2]" value="source"></td>
    <td width="1" height="0" valign="top" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="183" height="0" valign="top" bgcolor="#FFFFFF"><?php echo nl2br ($sql[source]);?></td>
  </tr>
  <tr  style="display:none" >
    <td height="23" valign="top" bgcolor="#FFFFFF">Source URL: <strong style="color:#FF0000">*</strong></td>
    <td width="336" height="0" valign="top" bgcolor="#FFFFFF"><input name="lang_detail[3]" type="text" size="50" value="<?php echo select_lang_detail($_GET[id],$_GET[langid],'sourceLink','article_list');?>">
      <input type="hidden" name="lang_field[3]" value="sourceLink"></td>
    <td width="1" height="0" valign="top" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="183" height="0" valign="top" bgcolor="#FFFFFF"><?php echo nl2br ($sql[sourceLink]);?></td>
  </tr>
...
//    ["Email","{%email%}"]];//Define custom tag selection
 ...
 //   oEdit1.customColors=["#ff4500","#ffa500","#808000","#4682b4","#1e90ff","#9400d3","#ff1493","#a9a9a9"];//predefined custom colors
    ...
    oEdit1.REPLACE("lang_detail[9]");
  	...
  	<tr>
    <td height="17" bgcolor="#FFFFFF">Link : </td>
    <td height="17" bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
                <tr valign="top"> 
                  <td width="28%" bgcolor="#FFFFFF"><input name="browsefile" type="radio" value="1" checked="checked">
                  ใส่ URL ของ web หรือ ไฟล์ </td>
            <td width="72%" bgcolor="#FFFFFF"><input name="link" type="text" id="link" size="45" value="<?php echo select_lang_detail($_GET[id],$_GET[langid],'link_html','article_list');?>" onFocus="document.form1.browsefile[0].checked=true" >
              <a href="#browse" onClick="win2 = window.open('../FileMgt/website_main.php?stype=link&Flag=Link&o_value=window.opener.document.form1.link.value','WebsiteLink','top=100,left=100,width=800,height=600,resizable=1,status=0');document.form1.browsefile[0].checked=true;win2.focus();"><img src="../images/i_folder_on.jpg" width="16" height="16" border="0" align="absmiddle" ></a></td>
                ...
                <tr valign="top">
                  ...
                  <td bgcolor="#FFFFFF"><input type="file" name="filebrowse"  value="<?php echo select_lang_detail($_GET[id],$_GET[langid],'link_html','article_list');?>" onClick="document.form1.browsefile[1].checked=true"></td>
                </tr>

              </table><input type="hidden" name="lang_field[9]" value="link_html">
      <input type="hidden" name="news_use" value="<?php echo $sql["news_use"] ;?>"></td>
    <td  valign="top" bgcolor="#FFFFFF">&nbsp;</td>
    <td  height="0" valign="top" bgcolor="#FFFFFF"><?php echo $sql[link_html];?></td>
  </tr>
  ...
  <?php } ?>
   ...
   <tr>
    ...
    <td height="36" colspan="3" bgcolor="#FFFFFF"><input type="submit" name="Submit" value="บันทึก">
      
      <input type="hidden" name="flag" value="set_lang">
	  <input type="hidden" name="num" value="<?php if($sql[at_id] > 0 && $sql[at_id] < 10 && $sql["news_use"] =='2'){  echo ($n); }else if($sql[at_id]  == 10){  echo '10';}else if($sql["news_use"] == '1'){  echo '10';}else{ echo '9';}?>">
