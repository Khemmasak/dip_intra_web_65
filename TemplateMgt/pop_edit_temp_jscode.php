...
<form id="form_main" name="form_main" method="POST" action="<?=getLocation('func_edit_temp_jscode')?>" enctype="multipart/form-data" >
<input type="hidden" name="proc" id="proc" value="Edit_jscode">
...
<h4 class="modal-title color-white"> <i class="fab fa-js-square" aria-hidden="true"> </i> JavaScript Coding</h4>
</div>
...
<?php
$myfile = fopen("../Login/assets/js/custom.js", "r") or die("Unable to open file!");
while(!feof($myfile)) {
  //$css .= fgets($myfile);
...
<button onclick="JQEdit_JScode($('#form_main'));" type="button" class="btn btn-success  btn-ml " >
