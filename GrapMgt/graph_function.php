...
if($_POST["todo"] == "DelRow"){
	...
	<?php
			...
			if($_POST["Flag"] == "SaveGraph"){
				...
				$db->query("UPDATE graph_index SET 
				graph_subject = '{$subject}',
			    graph_description = '{$desc}',
				graph_type = '{$_POST[g_type]}',
				graph_bgcolor = '{$_POST[g_bgcolor]}',
				graph_width = '{$_POST[g_width]}',
				graph_height = '{$_POST[g_height]}',
				graph_align = '{$_POST[g_align]}',
				graph_bgpic = '{$pic_name}', 
				graph_x = '{$_POST[g_x]}',
				graph_y = '{$_POST[g_y]}'
				WHERE graph_id = '{$_POST[graph_id]}'  ");
...
$txt = '<graph  '.$graph_img.' caption=\''.stripslashes(ereg_replace("'",'`',$data['graph_subject'])).'\'  subcaption=\''.stripslashes(ereg_replace("'",'`',$data['graph_description'])).'\'  xAxisName=\'XXX\' yAxisName=\'YYY\' decimalPrecision=\'0\' formatNumberScale=\'0\' showNames=\'1\'  baseFont=\'Tahoma\'  baseFontSize=\'11\'  outCnvBaseFontSize=\'11\' outCnvBaseFont=\'Tahoma\' bgcolor=\''.$graph_bgcolor.'\'>';
	...
	@fclose($fp);
		...
		<script language="JavaScript">
		    ...
		    self.location.href ="graph_preview.php?graph_id=<?php echo $_POST["graph_id"]; ?>";
		</script>
	<?php
		}
$db->db_close(); ?>
