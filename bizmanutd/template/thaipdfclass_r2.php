...
*******************************************************************************/
		function ThaiPDF($orientation='P',$unit='mm',$format='a4'){
			...
			if(is_string($format))	{
				...
				elseif($format=='a4') $format=array($Bizwidth,$Bizheight);
				elseif($format=='a5') $format=array(420.94,595.28);
				elseif($format=='carbon') $format=array(792,792);
	...
	//Define an alias for total number of pages
		...
		function BlankPage(){
			...
			$num = 25;
				for($i=0;$i<40;$i++){
				$this->SetXY(10,$num);
				$this->Cell('','',('________________________________________________________________________________________________________'));
				$num += 6;
				}
		...
		function Header(){
			global $division_name,$pg;
			if($this->HAdd == 1){
    			//Select Arial bold 15
				$this->SetFont('CordiaNew','',14);
				$this->SetXY(275,8);
				$this->Cell('','',('หน้าที่ '.$pg));
				$this->SetXY(80,10);
				$this->SetFont('CordiaNew','B',18);
				$this->Cell('','',('ข้อมูลทะเบียนผู้ใช้งานระบบบริการข้อมูลอิเล็กทรอนิกส์/จดหมายอิเล็กทรอนิกส์'));
				$this->SetXY(130,15);
				$this->Cell('','',('กรมทรัพยากรธรณี' ));
				$this->SetXY(10,25);
				$this->SetFont('CordiaNew','',16);
				$this->Cell('','',($division_name));
				$this->SetXY(10,29);
				$this->SetWidths(array(180,97));
				$this->Row(array("ชื่อ - นามสกุล","ลงชื่อผู้รับ"));
				// หัวตาราง
			 }
		}
