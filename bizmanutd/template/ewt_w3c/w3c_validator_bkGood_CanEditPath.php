					...
					$sqlChkColor = "  SELECT 
						...
						if($_POST["run_edit"]  ||  $total_err == 0 ) { // แก้ไขเสร็จ ก็รวม tag กลับเป็นหน้าเว็บเพจ w3c																	
									...
									while($rec_edit_path = $db2->fetch_array($exec_edit_path))	{
											...
											*/
													if( $arr_path[0] && (substr_count($arr_path[0], '.') < 3) && !eregi("^#",$arr_path[0]) ) { 
														...
														$edit_value = $charac2."../../".str_replace("../","",$rec_edit_path[text_attr_value1]).$charac2;
														$UPDATE = " UPDATE web_attr SET  text_edit_value = '$edit_value' WHERE text_attr_name = '".$rec_edit_path[text_attr_name]."' AND text_id = '".$rec_edit_path[text_id]."'   ";
														 //echo "$UPDATE<br>";
														 $db2->query($UPDATE);	
													}
									...
									while($rec_edit_path = $db2->fetch_array($exec_edit_path))	{
										...
										if($arr_path[1]) {
												...
												if( $arr_tail[0]  && ((substr_count($arr_tail[0], '.') < 3) && ( !eregi("^http:", $arr_tail[0]) ) && !eregi("^www.", $arr_tail[0])) || (eregi($CS_WWW, $arr_tail[0]) &&  !eregi("/w3c", $arr_tail[0]) )  ) {
													...
													$edit_value = $charac2.$arr_path[0].":url(../../".str_replace("../","",$arr_tail[0]).")".$arr_tail[1].$charac2;
				   ...
				   if($_GET["page_type"]==2) {  // Template 
							...
							if(eregi("id=\"ewt_main_structure_body\"" ,$new_contents)) {
									...
									if($arr_check2[1] ) {																								
											...
											//echo "arr_check2[1] : ".trim(htmlspecialchars($arr_check2[1]));
											
											// ต้องมีเว้นวรรคใน value="##" > str_replace จึงจะ หาเจอ  เพราะ tidy มันดันจัดเว้นวรรคให้ 
											$clear_spliter = str_replace('<input name="w3c_spliter" type="hidden" value="##" >','',$arr_check2[1]);										
											$spliter  = '<input name="w3c_spliter" type="hidden" value="##" >'.$clear_spliter;
											
											//echo htmlspecialchars($spliter);
											//echo "<BR>";
											$new_contents = $arr_check[0]."id=\"ewt_main_structure_body\"".$arr_check2[0].">".$spliter;																							
