		...
		<tr>
			<th bgcolor="#F9F9F9" scope="col" valign="top"><br>
				<table width="90%" style="height:30px" border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
					<tr><th style="height:30px" bgcolor="#F4F4FF" align="left">&nbsp;<img src="mainpic/icon_more.gif" width="12" height="7"  alt="icon_more"> รายชื่อผู้ที่เกี่ยวข้อง</strong></th></tr>
					<tr>
									...
									<th scope="col">
										<div align="left">&nbsp;
											<select name="search_select" id="search_select" onChange="if(this.value == ''){document.getElementById('search_text').style.display = 'none'; }else{document.getElementById('search_text').style.display = '';}">
												...
												<option value="1" <? if($_POST[search_select] == "1"){print " selected ";}?>> ชื่อ - นามสกุล</option>
												<option value="2" <? if($_POST[search_select] == "2"){print " selected ";}?>>หน่วยงาน</option>
											</select>&nbsp;
										...
										<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
												...
												<th align="center" valign="top" bgcolor="#FFFFFF" scope="col">
													<table width="98%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" align="center">
														<tr>
															...
															<td style="height:25px" bgcolor="#F4F4FF"><div align="center">ชื่อ-นามสกุล</strong></div></td>
														</tr>
														<?php
															$page = $_POST[page];
															if(!$limit) $limit = 10;
														...
														<td style="height:25px" bgcolor="#FFFFFF">
															<div align="center">
																<input name="chk<?=$result[gen_user_id]?>" type="checkbox" id="chk<?=$result[gen_user_id]?>" value="<?=$result[gen_user_id]?>" 
																onClick="
																if(this.checked) {
																	selectedStaff('<?=$result[gen_user_id]?>', '<?=$result[title_thai]." ".$result[name_thai]." ".$result[surname_thai]?>');
																} else {
																	delSelected('<?=$result[gen_user_id]?>');
																}" 
																<? if(count($_POST[id]) > 0) foreach($_POST[id] as $value){ if($result[gen_user_id] == $value) print "checked";}?>
																>
																<script language="javascript" type="text/javascript">
																<? if(count($_POST[id]) == 0 && count($_POST[name]) == 0 && $_POST[Submit2] != "บันทึก"){?>
																	if( invite_id_chk.length > 0){
																		for(var id_chk in invite_id_chk){
																			var chk_chk = '<?=$result[gen_user_id]?>';
														...
														?><?=$result[title_thai]." ".$result[name_thai]." ".$result[surname_thai]?>&nbsp;(<?=$text_org?>)</td></tr>
														<? 
															}
														?>
													<tr>
											...
											<th width="250" align="center" valign="top" bgcolor="#FFFFFF" scope="col">
												<table width="98%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" align="center" id="selectedTbl">
													<tr>
														<td style="height:25px" bgcolor="#F4F4FF" align="center">รายชื่อที่ถูกเลือก</strong></td>
													    <td width="30" align="center" bgcolor="#F4F4FF">ลบ</strong></td>
													</tr>
													<?php
			...
			<table width="100%" border="0" align="center" cellspacing="1">
				<tr><th scope="col"><input type="button" name="Submit2" value="บันทึก" onClick="submitStaff()"><input type="button" name="Submit3" value="ยกเลิก" onClick="window.close();//clear_chkbox(1,<?=$i?>,'chk');"></th></tr>
			</table>
		</th>
	</tr>
</table>
