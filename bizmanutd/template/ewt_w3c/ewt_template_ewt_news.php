...
$_SESSION["EWT_TYPE_ID"];
	...
	$sql_person = $db->query("SELECT * FROM personal_map WHERE p_group = '".$_SESSION["EWT_TYPE_ID"]."' ");
		if($db->db_num_rows($sql_person) > 0){
			...
			if($_GET["nid"] != ""){
			$sql_index = $db->query("SELECT d_id_w3c,d_id,c_id FROM article_list WHERE n_id = '".$_GET["nid"]."'");
			$XX = $db->db_fetch_array($sql_index);
			$c_id1 = $XX[c_id];
			echo $XX[d_id_w3c];
				if($XX[d_id_w3c] == '0' || $XX[d_id_w3c] == ' '){
				//$XX[0] = $XX[1];
					$sql_indexC = $db->query("select d_id_w3c,d_id from  article_group where c_id = '".$XX[c_id]."'");
					$XXC = $db->db_fetch_array($sql_indexC);
					if($XXC[d_id_w3c] == '0' || $XXC[d_id_w3c] == ' '){
					//$XX[0] = $XX[1];
						$sql_index_defC = $db->query("SELECT d_id FROM design_list WHERE d_default_w3c = 'Y'  ");
						$FF_dfC = $db->db_fetch_array($sql_index_defC);
						$XX["d_id_w3c"] = $FF_dfC["d_id"];
					}else{
						$XX["d_id_w3c"] = $XXC["d_id_w3c"];
					}
				}
				if($XX[d_id_w3c] != '0'){
...
$c_id1 = $XX[c_id];
	...
	if($XX[d_id_w3c] != '0' || $XX[d_id_w3c] != '0'){
	$sql_index = $db->query("SELECT d_id FROM design_list WHERE d_id = '$XX[d_id_w3c]'  ");
	}else{
		if($_GET["filename"] != ""){
			$sql_index = $db->query("SELECT template_id_w3c,template_id FROM temp_index WHERE filename = '".$_GET["filename"]."' ");
...
$d_idtemp =select_lang_detail($c_id1,$lang_shxxx[1],'d_id_w3c','article_group');
}
...
$file_template =$Website.$folder .'/ewt_preview_template_view.php?filename='.$_GET[filename].'&n_id='.$_GET["nid"].'&filenametemplate='.$template_w3c;
