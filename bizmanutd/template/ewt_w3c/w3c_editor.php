...
/*  ยกเลิกการรวมหน้าแล้ว จึงไม่ต้องสร้าง folder mainbkup
...
$sql_info = " SELECT  cms_description, w3c_html, mode_edit FROM  webpage_info  WHERE filename = '$filecheck' AND db_name = '".$main_db."' AND page_type = '".$_GET["page_type"]."'  ";
	...
	if($_POST["accept"]=="save") {
			...
			$edited_phpcontents = eregi_replace("–",'-',$edited_phpcontents);
			$edited_phpcontents = eregi_replace("  "," ",$edited_phpcontents);  // แก้เว้นวรรคเกิน 1 
			$edited_phpcontents = eregi_replace(" >",">",$edited_phpcontents); // แก้เว้นวรรคเกิน 1 
			$edited_phpcontents = eregi_replace("</object>","<\/object>",$edited_phpcontents); // แก้เว้นวรรคเกิน 1 
			...
			if($_POST["useTemplate"]) {
			
					$UPDATE2 = " UPDATE webpage_info SET  use_template_name = '".$_POST["useTemplate"]."', modify_time  = NOW()  WHERE  filename = '$filecheck' AND db_name = '".$main_db."' AND page_type = '".$_GET["page_type"]."' ";
					//echo "$UPDATE<br>";
					$db2->query($UPDATE2);
...
echo $labelShow; ?>
 ...
 <img src="images/photo_portrait.gif" alt="กรอกคำบรรยายภาพ" border="0"  align="absmiddle" onClick="
             ...
             } "  style="cursor:pointer"> กรอกคำบรรยายภาพ 
 </td></tr></table>
<br>
...
<span id="sp_mode2" style=" font-family:MS Sans Serif; font-size:10pt">
	<textarea name="htmlcode"  id="htmlcode"  cols="100" rows="25" wrap="off" ><?php 
	 function encodeHTML($sHTML)
		...
		if($rec_info["w3c_html"]) {
			echo encodeHTML(stripslashes($htmlcode));
		}
	?></textarea>
</span>
</td>
...
<input name="close" type="button" value="       ปิดหน้าต่าง      " onclick="window.close();"></form>
   <script type="text/javascript"> 
   createTextAreaWithLines('htmlcode'); 
   </script> 
<?php 
$db2->close_db();
$db->db_close();		
