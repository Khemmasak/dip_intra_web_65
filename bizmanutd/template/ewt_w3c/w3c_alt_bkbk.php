...
if(file_exists($dir3)) {
	@unlink($dir3.$filecheck.".php");  // ลบไฟล์หน้าหลักอันเก่า ทิ้งก่อน
}
...
if($numCheck==0 &&  !$_POST["run_edit"] ) {   // ถ้ายังไม่มีการโหลด content จากหน้าที่แปลง W3C แล้ว  และยังไม่ได้กด บันทึก alt
			...
			$arr_contents = split("</body>",$arr_temp2[1],2);
			//print_r($arr_temp);
			//echo "<br>";
			//print_r($arr_temp2);
			//echo "<br>";
							...
							if( strlen($arr_tag[$i]) <= 50 && trim($arr_tag[$i]) != "!doctype" ) { // เอาเฉพาะ ชื่อ tag จริงๆ มันต้องมี เว้นวรรค แล้วความยาวชื่อไม่เกิน 50 และไม่เอา DOCTYPE 
									...
									if($in_script1==false && $in_script2==false ) { // ถ้า code แต่ละ record ที่ผ่านมาไม่ใช่  code ในช่วงของ tag script จึงจะ insert									
										...
										$INSERT = " INSERT INTO web_tag_html (filename, original_url, db_name, page_type, text_tag, text_value, text_attr, text_rank, section_edit) 
																		VALUES ('$filecheck', '', '".$main_db."', '".$_GET["page_type"]."', '".$disp->convert_qoute_to_db(trim($arr_tag[$i]))."', '".$disp->convert_qoute_to_db($text_value[$i])."', '".$disp->convert_qoute_to_db($attribute_pack[$i])."', '$rank', '4' )";
										$db2->query($INSERT);
 ...
 if($_POST["run_edit"]) {
			...
			if($_GET["page_type"]==2) {   // Template 
						...
						$head_contents = file_get_contents("style.html");  // head section														
								...
								$sql_chk_title = " SELECT  text_id, text_tag, text_value, text_rank  FROM  web_tag  WHERE filename = '$filecheck' AND db_name = '$main_db' AND page_type = '".$_GET["page_type"]."' AND text_tag = 'title' AND  ( text_status <> 'del'  OR text_status is null )  ";										
								$exec_chk_title = $db2->query($sql_chk_title);	
								$rec_chk_title = $db2->fetch_array($exec_chk_title);	
			
								$head_contents.="<title>".$disp->convert_specials_char($rec_chk_title[text_value])."</title>";
														...
														if($rec_w3c[text_tag]) {
																	...
																	if(!eregi("^/",$rec_w3c[text_tag])) { // ถ้าไม่ใช่ tag ปิด
																		...
																		if($rec_w3c[section_edit]==3) {  //  head 
																			...
																			$head_contents.="<".$disp->convert_specials_char($rec_w3c[text_tag])." ".eregi_replace("(&amp;)","&",$text_up_attr).">".ereg_replace("&amp;",'&',$rec_w3c[text_value]);
																			
																		} else {	// ถ้าเป็น section 4 body  หรือ ไม่รู้ section ไหน ให้เก็บไว้ในตัวแปร body $new_contents ไว้ก่อน
																			if($text_up_attr) {
																				$new_contents.="<".$disp->convert_specials_char($rec_w3c[text_tag])." ".eregi_replace("(&amp;)","&",$text_up_attr).">".ereg_replace("&amp;",'&',$rec_w3c[text_value]); 
																				 //  ereg_replace("&amp;",'&',$new_contents)
																			} else {																			
				...
				/*		$sql_check = " SELECT  * FROM  webpage_info  WHERE filename = '$filecheck' AND db_name = '".$main_db."' AND page_type = '".$_GET["page_type"]."'    ";
						...
						ป้องกัน logo เบิ้ลเวลารวม template แล้ว
				*/
			if($_GET["page_type"]==2) {	  // Template 			
						$phpcontents = "<?php  //include session or connect ?>".$dtd_html_head_charset_top.$head_contents.$dtd_html_head_charset_bottom.$new_contents."</body>".$END_PAGE;					
						if(file_exists($dir2)) {									
								$result1 = $disp->testvar_infile($phpcontents, $dir2.$filecheck.".php", "w");// จึงต้อง write file แืืทน 
						}
			} else {	// BODY
						$phpcontents = "<?php  //include session or connect ?>".$dtd_html_head_charset_top.$head_contents.$dtd_html_head_charset_bottom." <body>".$COMPTOP.$new_contents.$COMPBOTTOM."</body>".$END_PAGE;
