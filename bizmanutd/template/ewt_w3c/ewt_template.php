		...
		$lang_sh = explode('___',$_GET["filename"]);
			if($lang_sh[1] != ''){$lang_sh = '_'.$lang_sh[1];}else{$lang_sh ='';}
				if($_GET["filename"] != ""){
					...
					$sql_index2 = $db->query("SELECT template_id_w3c,template_id FROM temp_index WHERE filename = '".$_GET["filename"]."' ");
					$XX = $db->db_fetch_array($sql_index2);
...
$file_template =$Website.$folder .'/ewt_preview_template_view.php?filename='.$_GET[filename].'&filenametemplate='.$template_w3c;
