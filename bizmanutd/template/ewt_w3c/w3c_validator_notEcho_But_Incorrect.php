 ...
 ( text_rank < ( SELECT MIN(text_rank) FROM web_tag WHERE filename = '$filecheck' AND db_name = '$main_db'  AND text_tag = '/".$rec_notbe[donot_be_inside]."'  AND text_rank > x1 ) )  ORDER BY text_rank, text_id  */
	 ...
	 ( text_rank > ( SELECT MAX(text_rank) AS max_open FROM web_tag WHERE filename = '$filecheck' AND db_name = '$main_db' AND ( text_status <> 'del' OR text_status is null ) AND text_tag = '".$rec_tag_group[section_name]."'  AND text_rank < x1 ) )  AND ( ( text_rank < ( SELECT MIN(text_rank) AS min_close FROM web_tag WHERE filename = '$filecheck' AND db_name = '$main_db' AND ( text_status <> 'del' OR text_status is null ) AND text_tag = '/".$rec_tag_group[section_name]."'  AND text_rank > x1 )  )	)	
						 ...
						 if($_POST["run_edit"]) { 		// ส่วนการบันทึก การลบ tag ที่ต้องการลูก  แต่ในหน้าเว็บ ไม่มีการใส่ tag ลูก
						 			...
						 			for($ct=1;$ct<=$_POST["total_tg"];$ct++) {
											if($_POST["chktag_del".$ct]) {
														...
														// หา text_rank ของ tag เปิด ที่จะลบก่อน																
													  $sql_id = " SELECT text_id, text_tag, text_rank  FROM  web_tag  WHERE filename = '$filecheck' AND db_name = '$main_db' AND   text_id = '".$_POST["chktag_del".$ct]."' ";																				
														$exec_id  = $db2->query($sql_id);
														$rec_id = $db2->fetch_array($exec_id);
														$text_rank_open = $rec_id[text_rank];				
														...
														if($rec_close[text_id] && $rec_close[text_tag]=="/".$rec_id[text_tag]) {   // ถ้ามี tag ปิด
																...
																$sql_next_id = " SELECT text_id, text_tag, text_rank FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '".$rec_id[text_tag]."'  AND  ( text_status <> 'del'  OR text_status is null ) AND  text_rank = ( SELECT  MIN(text_rank) AS min_rank  FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '".$rec_id[text_tag]."' AND  ( text_status <> 'del'  OR text_status is null )  AND text_rank > '$text_rank_open' ) ";
														...
														$or_edit_id = "";
										 				 while($recRequireChild = $db2->fetch_array($execRequireChild)) {		
																 ...
																 for($dd=0;$dd<count($arr_child);$dd++) {																 																		 																		 
																		 $sql_edit_id = " SELECT text_id, text_tag, text_rank FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '".trim($arr_child[$dd])."'  AND  ( text_status <> 'del'  OR text_status is null ) AND  text_rank = ( SELECT  MIN(text_rank) AS min_rank  FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '".trim($arr_child[$dd])."' AND  ( text_status <> 'del'  OR text_status is null )  AND text_rank > '$text_rank_open' $filter_edit_id) ";
						...
						while($recTagParent = $db2->fetch_array($execTagParent)) {
								  ...
								  $execRequireChild = $db2->query($sqlRequireChild);																												  								  //echo "$sqlRequireChild<br>";
								  
								  $childMissing = false;
								  
								  $sql1 = " SELECT text_id, text_tag, text_rank FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '".$recTagParent[tag_parent]."'  AND  ( text_status <> 'del'  OR text_status is null )  ORDER BY  text_rank  ";
								  $exec1 = $db2->query($sql1);								  
								  while($rec1=$db2->fetch_array($exec1)) {								  		   
										  ...
										  $sqlCloseParent = "  SELECT MIN(text_rank) AS min_rank FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '/".$recTagParent[tag_parent]."'  AND  text_rank > '$rank_parent'  AND  ( text_status <> 'del'  OR text_status is null ) ";
										 ...
										 if($haveCloseParent) {    // ถ้ามี tag ปิด  ของ tag ที่จะลบ ซึ่งใกล้ที่สุด
												...
												if($rankCloseParent>0) {
										 		 ...
										 		 $sqlNextParent = "  SELECT MIN(text_rank) AS min_rank FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  AND  text_tag = '".$recTagParent[tag_parent]."'  AND  text_rank > '$rank_parent'  AND  ( text_status <> 'del'  OR text_status is null ) ";
										  ...
										  while($recRequireChild = $db2->fetch_array($execRequireChild)) {										  																 													 $ableChildren .= $recRequireChild[tag_child].", ";				 													 				 
													 ...
													 $sql2 = " SELECT MIN(text_rank) AS min_rank  FROM  web_tag  WHERE   filename = '$filecheck' AND db_name = '$main_db'  $filter_child  AND  text_rank > '$rank_parent'  AND  ( text_status <> 'del'  OR text_status is null ) $filterCloseParent ";
													  //echo "sql2 : $sql2<br>";
				  ...
				  <?php 
					...
					$sqlChkColor = "  SELECT 
						...
						if($_POST["run_edit"]  ||  $total_err == 0 ) { // แก้ไขเสร็จ ก็รวม tag กลับเป็นหน้าเว็บเพจ w3c																	
								...
								/*  ยังไม่รู้จะแก้ path ใน SWFObject  ยังไง  มันยากมาก เพราะเป็น code javascript ที่ไม่ตายตัว  เหมือน HTML
								if(eregi("new SWFObject(", $new_contents)) {
										if(eregi("SWFObject(\"../", $new_contents)) {
