					...
					<table width="99%" border="0" align="center" cellpadding="10" cellspacing="1" bgcolor="#CCCCCC">
					<?php
					$b = 0;
...
<input type="button" name="save" value=" บันทึก " style="width:60 px"  onClick="sendCode('dx<?php echo $a; ?>y<?php echo $b; ?>',document.formex<?php echo $a; ?>y<?php echo $b; ?>,document.all.pex<?php echo $a; ?>y<?php echo $b; ?>)"><input type="button" name="cancel" value=" ยกเลิก " style="width:60 px"    onClick="cancelCode('dx<?php echo $a; ?>y<?php echo $b; ?>',document.formex<?php echo $a; ?>y<?php echo $b; ?>,document.all.pex<?php echo $a; ?>y<?php echo $b; ?>)"></div>
						  ...
						  </span><iframe name="ftargetx<?php echo $a; ?>y<?php echo $b; ?>" id="ftargetx<?php echo $a; ?>y<?php echo $b; ?>" width="0" height="0"></iframe>
						  </td>
						  <?php
						  $sql_r = $db->query("SELECT ad_id,ad_des FROM article_detail WHERE n_id = '$nid' AND at_type_row = '$x' AND at_type_col = '2' ");
						  $C1 = $db->db_fetch_array($sql_r);
						  $a = "2";
						  ?>
						<td width="65%" valign="top" >
<script type="text/javascript">
function generate_x<?php echo $a; ?>y<?php echo $b; ?>(){
		var oEditx<?php echo $a; ?>y<?php echo $b; ?> = new InnovaEditor("oEdit<?php echo $a; ?><?php echo $b; ?>");
		
		oEditx<?php echo $a; ?>y<?php echo $b; ?>.width="400";
				oEditx<?php echo $a; ?>y<?php echo $b; ?>.height="250";
		oEditx<?php echo $a; ?>y<?php echo $b; ?>.tabs=[
    ["tabHome", "", ["grpFont", "grpPara"]]
    ];

    oEditx<?php echo $a; ?>y<?php echo $b; ?>.groups=[
    ...
    ["grpPara", "", ["Paragraph", "Indent", "Outdent", "LTR", "RTL", "BRK", "JustifyLeft", "JustifyCenter","JustifyRight","JustifyFull", "Numbering","Bullets", "XHTMLSource"]]
		...
		oEditx<?php echo $a; ?>y<?php echo $b; ?>.mode="HTMLBody"; //Editing mode. Possible values: "HTMLBody" (default), "XHTMLBody", "HTML", "XHTML"
		
		oEditx<?php echo $a; ?>y<?php echo $b; ?>.REPLACE("dx<?php echo $a; ?>y<?php echo $b; ?>");
						  }
		</script><span  id="previewex<?php echo $a; ?>y<?php echo $b; ?>"><form action="article_editor.php" method="post"  name="formex<?php echo $a; ?>y<?php echo $b; ?>" target="ftargetx<?php echo $a; ?>y<?php echo $b; ?>"><strong>รายละเอียด: 
						<input name="ad_id" type="hidden" id="ad_id" value="<?php echo $C1["ad_id"]; ?>">
						<input name="nid" type="hidden" id="nid" value="<?php echo $nid; ?>">
						<input name="hidp" type="hidden" id="hidp" value="320">
						<input name="widp" type="hidden" id="widp" value="400">
						<input name="hide" type="hidden" id="hide" value="250">
						...
						<input name="posdisp" type="hidden" id="posdisp" value="x<?php echo $a; ?>y<?php echo $b; ?>">
						  ...
						  <textarea style="display:none" name="dx<?php echo $a; ?>y<?php echo $b; ?>" cols="40" rows="5" wrap="VIRTUAL" id="dx<?php echo $a; ?>y<?php echo $b; ?>"><?php
							  ...
							  ?></textarea>
							<table width="400" height="320" cellpadding="3" cellspacing="2" border="0" style="cursor:hand;border:#999999 dashed 1px;" onmouseover="this.style.borderColor='#FF0000'" onmouseout="this.style.borderColor='#999999'" title="Click To Edit"  onClick="this.style.display='none';generate_x<?php echo $a; ?>y<?php echo $b; ?>();document.all.hbOtx<?php echo $a; ?>y<?php echo $b; ?>.style.display='';">
							  <tr>
								<td bgcolor="#FFFF99"  valign="top" onmouseover="this.style.backgroundColor='#FFCCCC';" onmouseout="this.style.backgroundColor='#FFFF99';"><?php
								  if(trim($C1["ad_des"]) != "")
									{
