								 ...
								 $atf_array = array();
									for($q = 0;$q<count($dt);$q++){
																		...
																		$filename1 = ereg_replace($dt[$q],"<font color=\"#FF0000\">".$dt[$q]."</font>", $filename1);
																		 $sql_attf = "select attach_file_id from attach_file where meeting_id ='".$RS[meeting_id]."' and attach_fulltext REGEXP '".$dt[$q]."'";
...
?>
  ...
  if($atf_array != ''){
   ...
   if($afid != ""){
  $sql_attf = "select * from attach_file where attach_file_id in  (".$afid.") ";
  echo $sql_attf;
  $query_attf = $db->query($sql_attf);
...
$count_num = 0;
  ...
  echo "<div><img src=\"mainpic/page.jpg\"  align=\"absmiddle\"> <font color=\"#006600\"><b>".$typereport."</b></font> <br>";
  if($count_num > 0){
  echo "ค้นพบ ".number_format($count_num,0)." แห่ง &nbsp; [<a href=\"##F\" onClick=\"window.open('main_warehouse_view.php?attach_file_id=".$AFF[attach_file_id]."&keyword=".urlencode(base64_encode (implode(",", $CT_Word)))."', 'select_org', 'status=yes, menubar=no, scrollbars=yes, resizable=yes, height=450, width=600, left=150,top=100');\">ดูรายละเอียดเพิ่มเติม</a>] ";
  }
	echo "[<a href=\"##F\" onclick=\"download('".$AFF[attach_file_id]."','".$RS[meeting_id]."');\" >ดาวน์โหลดเอกสาร</a>]</div>";  
  }
  ?></font><br><br></td></tr>
   <?php
  }
