...
$global_theme = $R["d_bottom_content"];
 ...
 $sel = "SELECT * FROM docload_list 
 WHERE  
 ( dl_name like '%$keyword%'  OR  
 dl_detail like '%$keyword%'  OR  
 dl_filesize  = '".number_format(($keyword*1)*1024,0)."'  OR  
 dl_type like '%$keyword%'  OR  
 dl_update like '%$keyword%'  OR  
 dl_createdate like '%$keyword%' ) 
 AND dl_open='Y' ORDER BY dl_createdate";
...
?>
 <tr>
   ...
   <td width="60%" height="25" align="right" valign="top">
      <input type="text" name="keyword" class="styleMe" value="<?php echo $keyword;?>"> 
