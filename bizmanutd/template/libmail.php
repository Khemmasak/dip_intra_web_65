...
function From( $from )
{
	...
	$this->xheaders['From'] = "webmaster<webmaster@nhanet.or.th>";
}
...
//	$this->xheaders['To'] = implode( ", ", $this->sendto );
	...
	if( $this->charset != "" ) {
		...
		$this->xheaders["Content-Type"] = "text/html; charset=UTF-8$this->charset";
	...
	while( list( $hdr,$value ) = each( $this->xheaders )  ) {
		...
		$this->randf = $this->randc(5);
	@mkdir ("../tmp", 0777);
	$this->strTo = implode( ", ", $this->sendto );
	$fp = fopen("../tmp/t".$this->randf.".text", 'w');
	fwrite($fp, $this->strTo);
	fclose($fp);
	$fp = fopen("../tmp/b".$this->randf.".text", 'w');
	fwrite($fp, $this->fullBody);
	fclose($fp);
	$fp = fopen("../tmp/s".$this->randf.".text", 'w');
	fwrite($fp, $this->xheaders['Subject']);
	fclose($fp);
	$fp = fopen("../tmp/h".$this->randf.".text", 'w');
...
function Send()
{
	...
	$url = "http://192.168.142.19/autos1.php?url=".$this->randf;
...
function _build_attachement()
{
	...
	$this->fullBody .= "Content-Type: text/html; charset=UTF-8$this->charset\nContent-Transfer-Encoding: $this->ctencoding\n\n" . $this->body ."\n";
