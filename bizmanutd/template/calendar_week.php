<?php
	...
	if(!isset($display_week)) {
		$cur_date = substr($display_date, 8, 2);
		$cur_month = substr($display_date, 5, 2);
		$cur_year = substr($display_date, 0, 4);
		$display_dateinweek = date("w", mktime(0, 0, 0, $cur_month, $cur_date, $cur_year));
		if($display_dateinweek > 0) {
			$firstdateinweek = date('Y-m-d', mktime(0, 0, 0, $cur_month, $cur_date-($display_dateinweek-1), $cur_year));
		} else {
			$firstdateinweek = date('Y-m-d', mktime(0, 0, 0, $cur_month, $cur_date-6, $cur_year));
		}
		$first_date = substr($firstdateinweek, 8, 2);
		$first_month = substr($firstdateinweek, 5, 2);
		$first_year = substr($firstdateinweek, 0, 4);
		$enddateinweek = date('Y-m-d', mktime(0, 0, 0, $first_month, $first_date+6, $first_year));
		$end_date = substr($enddateinweek, 8, 2);
		$end_month = substr($enddateinweek, 5, 2);
		$end_year = substr($enddateinweek, 0, 4);
		$next_week = date('Y-m-d', mktime(0, 0, 0, $end_month, $end_date+1, $end_year));
		$prev_week = date('Y-m-d', mktime(0, 0, 0, $first_month, $first_date-7, $first_year));
	}
	...
	$sql_group_event  ="
		...
		where ";
			//(cal_event.event_date_start <= '".$enddateinweek."' and  cal_event.event_date_end >= '".$firstdateinweek."') and 
	$sql_group_event .=" (cal_event.event_date_start <= '".$enddateinweek."' and  cal_event.event_date_end >= '".$firstdateinweek."') ";
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td style="margin-left:0px; margin-right:5px;" nowrap="nowrap">
                        ...
                        <p class="text_normal" style="text-align:right;"><input type="text" name="txtsearch" id="txtsearch" value="<?php echo $_REQUEST['txtsearch']; ?>" />&nbsp;<input type="button" onClick="window.location.href='calendar_week.php?display_date=<?php echo $display_date;?>&sh=<?php echo $sh;?>&BID=<?php echo $BID;?>&txtsearch='+document.getElementById('txtsearch').value;" value="ค้นหา" /></p>
						<hr width="100%" size="1"  align="left"  color="#D8D2BD">
					</td>
				</tr>
			</table>
			...
			<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CECECE">
				<tr>
					<td bgcolor="#E7E7E7">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td align="right"><a href="calendar_week.php?display_date=<?php echo $prev_week;?>&sh=<?php echo $sh;?>"><img src="<?php echo $path_cal;?>mainpic/calendar/navi_left.gif" style="cursor:hand" align="absmiddle" border="0"></a></td>
								<td align="center" width="500">
									<span class="text_head">
										<?php echo  $head_test ;?>&nbsp;
										<?php//php echo $text_Gencalendar_textofday;?>
										&nbsp;<?php echo date('j', mktime(0, 0, 0, $first_month, $first_date, $first_year))?>&nbsp;<?php 
										switch($first_month) {
								...
								case 12:  echo $text_Gencalendar_m12; break;
										...
										?>&nbsp;<?php echo $text_Gencalendar_textpsyear;?>&nbsp;<?php echo (date('Y', mktime(0, 0, 0, $first_month, $first_date, $first_year))+$y_y);?>&nbsp;-&nbsp;<?php echo date('j', mktime(0, 0, 0, $end_month, $end_date, $end_year))?>&nbsp;<?php 
										switch($end_month) {
										case 1:  echo $text_Gencalendar_m1; break;
								case 2:  echo $text_Gencalendar_m2; break;
								case 3:  echo $text_Gencalendar_m3; break;
								...
								case 12:  echo $text_Gencalendar_m12; break;
										...
										?>&nbsp;<?php echo $text_Gencalendar_textpsyear;?>&nbsp;<?php echo (date('Y', mktime(0, 0, 0, $end_month, $end_date, $end_year))+$y_y);?>									</span>								</td>
								<td align="left"><a href="calendar_week.php?display_date=<?php echo $next_week;?>&sh=<?php echo $sh;?>"><img src="<?php echo $path_cal;?>mainpic/calendar/navi_right.gif" style="cursor:hand" align="absmiddle" border="0"></a></td>
							</tr>
						</table>
					</td>
				</tr>
				...
				<?php
					while($row_event = $db->db_fetch_array($result_event)) {
						...
						//ไม่ควรจะมีวันนะน่าจะแสดงแต่เวลา
