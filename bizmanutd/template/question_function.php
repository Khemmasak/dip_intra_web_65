...
include("lib/connect.php");
	...
	if($_REQUEST["wtid"]){ $_REQUEST["wtid"]=checkNumeric($_REQUEST["wtid"]); }
	...
	if($waid){ $waid = checkNumeric($waid); }
	if($_GET["waid"]){ $_GET["waid"] = checkNumeric($_GET["waid"]); }
	if($_POST["waid"]){ $_POST["waid"] = checkNumeric($_POST["waid"]); }
	if($_REQUEST["waid"]){ $_REQUEST["waid"]=checkNumeric($_REQUEST["waid"]); }
	
	if($vote){ $vote = checkNumeric($vote);  }
	if($_GET["vote"]){ $_GET["vote"] = checkNumeric($_GET["vote"]); }
	if($_POST["vote"]){ $_POST["vote"] = checkNumeric($_POST["vote"]); }
	 
	if($a_id){ $a_id = checkNumeric($a_id); }
	if($_GET["a_id"]){ $_GET["a_id"] = checkNumeric($_GET["a_id"]); }
	if($_POST["a_id"]){ $_POST["a_id"] = checkNumeric($_POST["a_id"]); }
	
	if($aemail){ $aemail=checkMails($aemail); }
	if($_GET["aemail"]){ $_GET["aemail"]=checkMails($_GET["aemail"]); }
	if($_POST["aemail"]){ $_POST["aemail"]=checkMails($_POST["aemail"]); }
	if($_REQUEST["aemail"]){ $_REQUEST["aemail"] =checkMails($_REQUEST["aemail"]); }
	//===============================================================================
...
if($flag == 'reqRemoveComment' or $flag == 'reqRemoveTopic'){
		$request_reason=addslashes(htmlspecialchars($request_reason));
		if($flag == 'reqRemoveTopic'){
			$Request_type='T';
			$wtid=$_POST['wtid'];
			$waid=$_POST['waid'];
		}else if($flag == 'reqRemoveComment'){
			$Request_type='A';
			$wtid=$_POST['wtid'];
			$waid=$_POST['waid'];
		}
		$amsg=$_POST['amsg'];
		$db->query("INSERT INTO  w_question_sts_request (t_id, a_id, request_createdate, request_lastdate, requestor_ip,request_reason,request_type,request_wid,request_email) VALUES('$wtid','$waid',NOW(), NOW(),'$IPn','$amsg','$Request_type','".$_SESSION["EWT_MID"]."','$aemail' ) ");

		?>
		<script language="JavaScript">
		window.location.href = "index_question.php?wcad=<?php echo $wcad; ?>&t=<?php echo $t?>&filename=<?php echo $_REQUEST[filename];?>";
		</script>
		<?php
}
