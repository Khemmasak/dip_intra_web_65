...
if( $_GET["tid"] != ""){
	...
	if(!session_is_registered("DL_VISITOR_STAT")){
		session_register("DL_VISITOR_STAT");
		$visitor = "Y";
		$myreferer = $_SESSION["DL_REFERER"];
		$sql_new = $db->query("SELECT COUNT(stat_id) FROM stat_download WHERE ip = '".$myip."' ");
...
if($visitor == "Y"){	
	...
	// end check OS
		...
		$db->query("USE datawarehouse");
		$db->query("INSERT INTO stat_download (data_wh_id,attach_file_id,timestamp,date,time,ip,country,
															language,new,browser,resolution,domain,isp,region,city,latitude,
															longitude,search,os) 
								VALUES ('".$_GET[mid]."','".$_GET[aid]."','".$mytimestamp."','".$mydate."','".$mytime."','".$myip."','".$mycountry."',
		...
		$db->query("USE datawarehouse");
				$db->query("INSERT INTO stat_download (data_wh_id,attach_file_id,timestamp,date,time,ip,country,
															language,new,browser,resolution,domain,isp,region,city,latitude,
															longitude,search,os) 
								VALUES ('".$_GET[mid]."','".$_GET[aid]."','".$mytimestamp."','".$mydate."','".$mytime."','".$myip."',NULL,
