...
//ยกเลิกการนับตรงนี้ให้ไปผูกกับ tor_list แทน
/*
		// check OS
...
if($_GET["tid"] != ""){
	...
	if($_SESSION["TOR_REFERER"]){
		session_register("TOR_VISITOR_STAT");
		$visitor = "Y";
		$myreferer = $_SESSION["TOR_REFERER"];
				
$db->query("USE db_moc_tor");
		$sql_new = $db->query("SELECT COUNT(ts_id) FROM tor_stat  WHERE ts_ip = '".$myip."' ");
...
if($visitor == "Y"){	
	...
	// end check OS
		$db->query("USE ".$EWT_DB_NAME);
$db->query("USE db_moc_tor");
		$db->query("INSERT INTO tor_stat (t_id,ts_timestamp,ts_date,ts_time,ts_ip,ts_country,
															ts_language,ts_new,ts_browser,ts_resolution,ts_domain,ts_isp,ts_region,ts_city,ts_latitude,
															ts_longitude,ts_search,ts_os) 
								VALUES ('".$_GET[tid]."','".$mytimestamp."','".$mydate."','".$mytime."','".$myip."','".$mycountry."',
		...
		$db->query("USE db_moc_tor");
				$db->query("INSERT INTO tor_stat (t_id,ts_timestamp,ts_date,ts_time,ts_ip,ts_country,
															ts_language,ts_new,ts_browser,ts_resolution,ts_domain,ts_isp,ts_region,ts_city,ts_latitude,
															ts_longitude,ts_search,ts_os) 
								VALUES ('".$_GET[tid]."','".$mytimestamp."','".$mydate."','".$mytime."','".$myip."',NULL,
