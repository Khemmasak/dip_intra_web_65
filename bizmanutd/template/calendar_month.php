<?php
	...
	if($firstdateinmonth > 0) { $firstdateincalendar = date('Y-m-d', mktime(0, 0, 0, $cur_month, 1-($firstdateinmonth-1), $cur_year));
	} else { $firstdateincalendar = date('Y-m-d', mktime(0, 0, 0, $cur_month, 1-6, $cur_year)); }
	...
	if($lastdateinmonth > 0) { $lastdateincalendar = date('Y-m-d', mktime(0, 0, 0, $cur_month+1, 0+(7-$lastdateinmonth), $cur_year));
	} else { $lastdateincalendar = date('Y-m-d', mktime(0, 0, 0, $cur_month+1, 0, $cur_year)); }
	...
	$sql_group_event  ="
		...
		where ";
			//(cal_event.event_date_start <= '".date('Y-m-d', mktime(0, 0, 0, $cur_month+1, 0, $cur_year))."' and  cal_event.event_date_end >= '".date('Y-m-d', mktime(0, 0, 0, $cur_month, 1, $cur_year))."') and 
	$sql_group_event .=" (cal_event.event_date_start <= '".date('Y-m-d', mktime(0, 0, 0, $cur_month+1, 0, $cur_year))."' and  cal_event.event_date_end >= '".date('Y-m-d', mktime(0, 0, 0, $cur_month, 1, $cur_year))."') ";
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td style="margin-left:0px; margin-right:5px;" nowrap="nowrap">
                        ...
                        <p class="text_normal" style="text-align:right;"><input type="text" name="txtsearch" id="txtsearch" value="<?php echo $_REQUEST['txtsearch']; ?>" />&nbsp;<input type="button" onClick="window.location.href='calendar_month.php?display_date=<?php echo $display_date;?>&sh=<?php echo $sh;?>&BID=<?php echo $BID;?>&txtsearch='+document.getElementById('txtsearch').value;" value="ค้นหา" /></p>
						<hr width="100%" size="1"  align="left"  color="#D8D2BD">
					</td>
				</tr>
			</table>
			...
			<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CECECE">
				<tr>
					<td bgcolor="#E7E7E7">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td align="right"><a href="calendar_month.php?display_date=<?php echo $prev_month;?>&sh=<?php echo $sh;?>"><img src="<?php echo $path_cal;?>mainpic/calendar/navi_left.gif" style="cursor:hand" align="absmiddle" border="0"></a></td>
								<td align="center" width="500">
									<span class="text_head">
										...
										?>&nbsp;<?php echo $text_Gencalendar_textpsyear;?>&nbsp;<?php echo (date('Y', mktime(0, 0, 0, $cur_month, $first_date, $first_year))+$y_y);?>									</span>								</td>
								<td align="left"><a href="calendar_month.php?display_date=<?php echo $next_month;?>&sh=<?php echo $sh;?>"><img src="<?php echo $path_cal;?>mainpic/calendar/navi_right.gif" style="cursor:hand" align="absmiddle" border="0"></a></td>
							</tr>
						</table>
					</td>
				</tr>
				...
				<?php
					while($row_event = $db->db_fetch_array($result_event)) {
						...
						while($rs_invite = $db->db_fetch_array($query_invite)){
							...
							//กรณีเป็นคน
							$sql_staff = "select title.title_id,title.title_thai,name_thai,surname_thai from gen_user inner join title on gen_user.title_thai = title.title_id where gen_user_id = '$rs_invite[person_id]' ";
				...
				<tr bgcolor="#FFFFFF" height="25">
					...
					<a href="javascript:void(0);" class="text_eng" onClick="window.open('calendar_editevent.php?event_id=<?php echo $row_event['event_id'];?>&ref=yes&sh=<?php echo $sh;?>','','width=800 , height=580, location=0');">
                      <!--<a href="calendar_editevent.php?event_id=<?php echo $row_event['event_id'];?>&ref=yes"> --><img src="<?php echo $path_cal?>mainpic/calendar_edit.gif" alt="Edit Event" width="16" height="16" border="0" align="absmiddle">&nbsp; <?php echo $text_general_edit;?> </a>&nbsp; <a href="javascript:if(confirm('คุณต้องการจะลบ Event นี้')) {
