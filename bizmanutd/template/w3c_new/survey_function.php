...
?>
   <script language="javascript">
       window.location.href="<?php if($PX[start_page]!=""){ echo $PX[start_page]; }else{ echo "survey_thank.php"; } ?>";
	</script>
<?php
}
if($allowUser>0){
...
?>
  ...
  <?php while($R=$db->db_fetch_array($SQL)){   ?>
  <?php
	if($R[c_gp] =="Y" ){
	?>
...
if($oth != ""){
		...
		$Path_true = '../survay_upload/survay'.$s_id;
...
$sql0 = "INSERT INTO ".$PR[s_table]." VALUES ('',NOW()";
$sql0 .= " ,'".$AllData."' ";
$sql0 .= ")";
$sql01 = $db->query($sql0);
//echo  $sql0;
