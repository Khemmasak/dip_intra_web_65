...
SELECT *
					FROM article_group 
					INNER JOIN n_group ON (article_group.c_id = n_group.g_name)  
					ORDER BY article_group.c_id asc LIMIT {$start} , {$perpage} 
*/
$_sql = $db->query("select * from article_group join n_group on n_group.g_name = article_group.c_id group by article_group.c_id, article_group.c_name order by article_group.c_name asc LIMIT {$start} , {$perpage}
					");
...
<a onClick="boxPopup('<?=linkboxPopup();?>pop_enews_cate_list.php');" data-toggle="tooltip" data-placement="top" title="<?=$txt_enews_setting_enews;?>">					
<button type="button" class="btn btn-success  btn-ml"   title="<?=$txt_enews_setting_enews;?>"  target="_self">
<i class="fa fa-cog"></i>&nbsp;<?=$txt_enews_setting_enews;?>
</button>
</a>
