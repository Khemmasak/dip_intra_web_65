<?php
	...
	if($_POST[flag]=='set'){
		$sql = "select * from email_config where module = 'calendar' ";
		$query = $db->query($sql);
		if($db->db_num_rows($query)>0){
			$db->query("UPDATE email_config SET email = '".$_POST[email]."' where   module = 'calendar' and status = '1'");
		}else{
		 $db->query("INSERT INTO email_config (module,email,status) VALUES ('calendar','".$_POST[email]."','1')");
		}
		print "<script>";
		print "alert('".$text_general_alertadd."');";
		print "window.location.href = \"calendar_config.php\";";
		print "</script>";
	}
	
	$sql = "select * from email_config where module = 'calendar' and status = '1' ";
	$query = $db->query($sql);
...
<table width="94%" border="0" align="center" cellpadding="0" cellspacing="0" class="ewtfunctionmenu">
  <tr>
    <td align="right"><a href="javascript:void(0);" onClick="load_divForm('../FavoritesMgt/favorites_add.php?name=<?php echo urlencode("ตั้งค่าปฏิทิน");?>&module=calendar&url=<?php echo urlencode("calendar_config.php");?>', 'divForm', 300, 80, -1,433, 1);"><img src="../images/star_yellow_add.gif" width="16" height="16" border="0" align="middle">&nbsp;Add to favorites </a>&nbsp;&nbsp;&nbsp;&nbsp;<hr>
    </td>
  </tr>
</table>
    ...
    <tr>
      <td width="87%" colspan="2" ><table width="70%" border="0" align="center" cellpadding="0" cellspacing="0" class="ewttableuse" style="border-collapse:collapse">
                ...
                <tr class="ewttablehead"> 
                  <td colspan="2"><span class="ewtfunction">ตั้งค่าปฏิทิน</span></td>
                </tr>
                <tr> 
                  <td width="28%" align="left" bgcolor="#FFFFFF">อีเมล์ของผู้ดูแลระบบปฏิทิน</td>
                ...
                <tr align="right"> 
                  ...
                  <input type="hidden" name="flag" value="set"></td>
                </tr>
              </table></td>
                  </tr>
              </table></td>
