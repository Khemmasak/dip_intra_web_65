...
$_sql = $db->query("SELECT * 
					...
					WHERE Complain_lead_ID = '{$com_cid}'
					");			  
$a_rows = $db->db_num_rows($_sql);	
$a_data = $db->db_fetch_array($_sql);
?>

<form id="form_main" name="form_main" method="POST" action="<?php echo getLocation('func_edit_complain_group')?>" enctype="multipart/form-data" >
<input type="hidden" name="proc" id="proc"  value="Edit_Cate">
<input type="hidden" name="com_cid" id="com_cid"  value="<?php echo $com_cid;?>">
<div class="container" >   
...
<h4 class="modal-title  color-white"><i class="fas fa-plus-circle"></i> <?php echo $txt_complain_edit_cate;?></h4>
</div>
...
<input class="form-control" placeholder="<?php echo $txt_complain_cate_title;?>" name="category_title" type="text" id="category_title"  value="<?php echo $a_data['Complain_lead_name'];?>" required="required" />
</div>
</div>
...
<div class="col-md-12 col-sm-12 col-xs-12" >
<input  class="form-control" placeholder="<?php echo $txt_complain_cate_email;?>" name="category_email" id="category_email"  value="<?php echo $a_data['Complain_lead_email'];?>"   required="required"  />
</div>
</div>
Comment -->
