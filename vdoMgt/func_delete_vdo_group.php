...
if($a_data['proc']=='DelGroup'){

$_sql = $db->query("SELECT *
					FROM vdo_group 
					WHERE vdog_id = '{$a_data['id']}'
					");
$a_rows = $db->db_num_rows($_sql);	
$a_data_group = $db->db_fetch_array($_sql);
$vdo_name = $a_data_group['vdo_name'];
	
del('vdo_group','vdog_id='.$a_data['id']);
...
$db->write_log("delete","vdo","ลบหมวดวีดีโอ ".$vdo_name);							   
//print_r($s_data);	
