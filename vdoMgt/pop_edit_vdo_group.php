...
$vdog_cid = (int)(!isset($_GET['vdog_id']) ? 0 : $_GET['vdog_id']);

$_sql = $db->query("SELECT * FROM vdo_group WHERE vdog_id = '{$vdog_cid}' ");
...
<form id="form_main" name="form_main" method="POST" action="<?= getLocation('func_edit_vdo_group') ?>" enctype="multipart/form-data">
	...
	<input type="hidden" name="vdog_image_old" id="vdog_image_old" value="<?= $a_data['vdog_image']; ?>">
	<input type="hidden" name="vdog_cid" id="vdog_cid" value="<?= $vdog_cid; ?>">
	<div class="container">
		<div class="modal-dialog modal-lg">
			...
			<div class="modal-content">
				<div class="modal-header ">
					...
					<h4 class="modal-title"><?= $txt_vdo_edit_cate; ?></h4>
				</div>
				...
				<div class="modal-body">
							...
							<div class="card-body">
								...
								<div class="form-group row ">
									...
									<div class="col-md-12 col-sm-12 col-xs-12">
										<input class="form-control" placeholder="<?= $txt_vdo_cate_name; ?>" name="vdog_name" type="text" id="vdog_name" value="<?= $a_data['vdog_name']; ?>" required="required" />
									</div>
								</div>
								<div class="form-group row ">
									<label for="vdog_info" class="col-sm-12 control-label"><b><?= $txt_vdo_cate_detail; ?> <span class="text-danger"><code>*</code></span> :</b></label>
									<div class="col-md-12 col-sm-12 col-xs-12">
										<textarea class="form-control" placeholder="<?= $txt_vdo_cate_detail; ?>" rows="6" id="vdog_info" name="vdog_info" required="required"><?= $a_data['vdog_info']; ?></textarea>
									</div>
								</div>
								<div class="form-group row ">
									<label for="vdog_img" class="col-sm-12 control-label"><b><?= $txt_vdo_img_cate; ?> <span class="text-danger"><code>*</code></span> :</b> </label>
									<div class="col-md-12 col-sm-12 col-xs-12">
										<input type="file" name="vdog_img" id="vdog_img" class="form-control" <?php if (empty($a_data['vdog_image'])) {
																													echo 'required="required"';
																												} ?> onchange="JSCheck_Img(this.id,this.value);" />
									</div>
									<br>
									<?php
									if ($a_data['vdog_image']) {
									?>
										<img src="<?= "../ewt/" . $_SESSION["EWT_SUSER"] . "/download/file_vdo/" . $a_data['vdog_image']; ?>" style="height: 60px;width: 60px;">
									<?php } ?>
									<span><?= $a_data['vdog_image']; ?></span><br>
					...
					<div class="modal-footer ">
						<div class="col-md-12 col-sm-12 col-xs-12 text-center">
							<button onclick="JQEdit_vdo_group($('#form_main'));" type="button" class="btn btn-success  btn-ml ">
...
<script>
	...
	function JQEdit_vdo_group(form) {
